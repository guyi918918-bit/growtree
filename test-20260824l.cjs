...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824l.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');

const BUILD = '20260824l';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 78) ok('SW CACHE 已递增到 v78'); else no('SW CACHE 未递增');
...
// 9. 历史详情表格样式存在
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824l.css'), 'utf8');
if (CSS.includes('.eng-history-detail .eng-manual-table') && CSS.includes('min-width:420px')) ok('历史详情表格对齐样式已添加'); else no('历史详情表格样式缺失');
