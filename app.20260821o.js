...
// Google Translate 走 CORS 代理（免 key，国内质量最稳）
async function engFetchGoogle(zh) {
    const base = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=zh-CN&tl=en&dt=t&q=${encodeURIComponent(zh)}`;
    const proxies = [
        u => u,
        u => `https://corsproxy.io/?url=${encodeURIComponent(u)}`,
        u => `https://api.allorigins.win/raw?url=${encodeURIComponent(u)}`,
...
// ---------- 6 列语法成分分析 ----------
function engParseSentence(en) {
    let clean = en.replace(/[.!?;:]+$/, '').trim();
    // 只解析第一个完整句（按 .!? 切），避免多句被全塞到宾语列
    const firstDotIdx = clean.search(/[.!?]/);
    if (firstDotIdx > 0) clean = clean.slice(0, firstDotIdx).trim();
    // 8 列结果：主语 / 谓语动词 / 宾语/表语 / 定语 / 状语时间 / 状语地点 / 状语方式 / 补足语
    let result = {
        subject: '—', verb: '—', object: '—', modifier: '—',
        advTime: '—', advPlace: '—', advManner: '—', complement: '—',
    ...
    // === Step 1: 抽取时间状语（句首） ===
    const timePatterns = [
        /\s(yesterday|today|tomorrow)\s/i,
        /\s(last|next|this)\s+(week|month|year|morning|afternoon|evening|night|weekend|sunday|monday|tuesday|wednesday|thursday|friday|saturday)\s/i,
        /\severy\s+(morning|afternoon|evening|night|day|week|month|year)\s/i,
        /\s\d+\s+(hours?|days?|weeks?|months?|years?)\s+ago\s/i,
        /\s(in|at)\s+the\s+(morning|afternoon|evening|night)\s/i,
    ];
    for (const pat of timePatterns) {
        const m = rest.match(pat);
        if (m) {
            result.advTime = m[0].trim();
            rest = rest.replace(m[0], ' ');
            break;
        }
    }

    // === Step 2: 抽取地点状语（at/in/on + 地点）===
    const placePatterns = [
        /\s(at|in|on)\s+(my\s+(?:place|home|house|school|office|work))\s/i,
        /\s(at|in|on)\s+(the\s+(?:park|school|store|shop|home|bank|beach|station))\s/i,
        /\s(at|in|on)\s+(?:Beijing|Shanghai|home|school|work|bed|table|desk|office)\s/i,
    ];
    for (const pat of placePatterns) {
        const m = rest.match(pat);
        if (m) {
            result.advPlace = m[0].trim();
            rest = rest.replace(m[0], ' ');
            break;
        }
    }

    // === Step 3: 抽取方式状语（疑问副词 / with + 伴随 / by + 工具 / 纯副词）===
    const mannerPatterns = [
        /^\s*(How|When|Where|Why)\s+/i,
        /\s(with|by)\s+\w+(?:\s+\w+){0,2}\s/i,
        /\s(slowly|quickly|carefully|well|hard|fast|often|usually|always|never|sometimes|easily|together)\s/i,
    ];
    for (const pat of mannerPatterns) {
        const m = rest.match(pat);
        if (m) {
            result.advManner = m[0].trim();
            rest = rest.replace(m[0], ' ');
            if (/^(How|When|Where|Why)$/i.test(result.advManner)) {
                result.isInterrogative = true;
            }
            break;
        }
    }

    // === Step 4: 抽取主语（疑问句无主语时不兜底取词） ===
    if (!result.isInterrogative) {
        const subjectPatterns = [
            /^\s*(my\s+husband\s+and\s+i|my\s+wife\s+and\s+i|my\s+friend\s+and\s+i|my\s+family\s+and\s+i)\s+/i,
            /^\s*(i|you|he|she|we|they)\s+/i,
            /^\s*(my|your|his|her|our|their)\s+\w+(?:\s+\w+){0,2}\s+/i,
            /^\s*(the|a|an)\s+\w+(?:\s+\w+){0,2}\s+/i,
            /^\s*[\w']+\s+/,
        ];
        for (const pat of subjectPatterns) {
            const m = rest.match(pat);
            if (m) {
                result.subject = m[0].trim();
                rest = rest.replace(m[0], ' ');
                break;
            }
        }
    ...
    // === Step 4.5: 优先识别"不定式 to + 动词 + 介词+名词"的补足模式 ===
    // 例如 "to stick to English" → verb=stick, complement="to English"
    const infMatch = rest.match(/^\s*to\s+(\w+)(?:\s+(to|at|in|for|about|with|from|on)\s+(\w+(?:\s+\w+){0,3}))?\s*$/i);
    if (infMatch && result.verb === '—') {
        result.verb = infMatch[1];
        if (infMatch[2]) result.complement = (infMatch[2] + ' ' + infMatch[3]).trim();
        rest = '';
    }

    // === Step 5: 抽取谓语动词（含复合短语）===
    const verbPatterns = [
        /^\s*went\s+mountain\s+climbing\s*/i,
        /^\s*went\s+swimming\s*/i,
        /^\s*went\s+shopping\s*/i,
        /^\s*(want|wanted|wants)\s+to\s+(have|eat|drink|see|go|do|talk|play|study|read)\s*/i,
        /^\s*(went|goes|go)\s+to\s+(eat|have|see|do|play|visit|drink)\s*/i,
        /^\s*(is|am|are|was|were)\s+\w+ing\s*/i,
        /^\s*(is|am|are|was|were)\s+\w+\s*/i,
        /^\s*(has|have|had|having)\s+/i,
        /^\s*(does|do|did|will|would|can|could|should|may|might|must)\s+/i,
        /^\s*(went|ate|had|ran|saw|said|told|spoke|gave|read|stuck|sticks|liked|loved|enjoyed|drinks|runs|likes|loves|plays|studies|reads|teach|teaches|learn|learns|stick|listen|listens|look|looks|talk|talks|think|thinks|get|gets|got|eat|eats|want|wants|go|goes|is|am|are)\s+/i,
        /^\s*\w+\s+/,
    ];
    for (const pat of verbPatterns) {
        const m = rest.match(pat);
        if (m) {
            result.verb = m[0].trim();
            rest = rest.replace(m[0], ' ');
            break;
        }
    }

    // === Step 6: 抽取补足语（动词 + 介词 + 名词，如 stick to English / listen to music）===
    // 仅在疑问句或主语已确定时尝试，避免误吃纯宾语
    const complementPatterns = [
        /^\s*\w+\s+(to|at|in|for|about|with|from|on)\s+\w+(?:\s+\w+){0,3}\s*$/i,
    ];
    for (const pat of complementPatterns) {
        const m = rest.match(pat);
        if (m && result.verb !== '—' && rest.trim().split(/\s+/).length <= 5) {
            result.complement = rest.trim();
            rest = '';
            break;
        }
    }

    // === Step 6.5: 抽取句尾残留的时间/地点/方式状语（任意位置都可识别）===
    // 解决 "I want to eat a bowl of noodles tonight" 中 tonight 漏到宾语列的问题
    const tailExtract = [
        { regex: /\s+(tonight|tomorrow|today|yesterday|now|soon|later)\s*$/i, key: 'advTime' },
        { regex: /\s+(every\s+(?:morning|afternoon|evening|night|day|week|month|year))\s*$/i, key: 'advTime' },
        { regex: /\s+(this|next|last)\s+(morning|afternoon|evening|night|week|month|year|weekend)\s*$/i, key: 'advTime' },
        { regex: /\s+(at\s+(?:my\s+(?:place|home|house|school|office|work)|the\s+(?:park|school|store|shop|home|bank|beach|station)|home|school|work|bed))\s*$/i, key: 'advPlace' },
        { regex: /\s+(in\s+(?:the\s+park|the\s+garden))\s*$/i, key: 'advPlace' },
        { regex: /\s+(with\s+(?:a|my|the)\s+\w+(?:\s+\w+){0,2})\s*$/i, key: 'advManner' },
        { regex: /\s+(by\s+\w+(?:\s+\w+)?)\s*$/i, key: 'advManner' },
        { regex: /\s+(slowly|quickly|carefully|well|hard|fast|often|usually|always|never|sometimes|together)\s*$/i, key: 'advManner' }
    ];
    for (const { regex, key } of tailExtract) {
        const m = rest.match(regex);
        if (m) {
            const v = m[0].trim();
            if (result[key] === '—') result[key] = v;
            else if (result[key] !== v) result[key] = result[key] + ' ' + v;
            rest = rest.replace(m[0], ' ');
        }
    }

    // === Step 7: 剩余作为宾语/表语（保留冠词，不强行去掉）===
    if (rest.trim()) {
        result.object = rest.trim();
    }
    return result;
}
...
function engGenerateOutput(zh, en) {
    ...
    const p = engParseSentence(cleanEn);
    const cell = v => v === '—' ? '—' : escapeHtml(v);
    const zhCell = v => {
        if (v === '—') return '—';
        const zh = engTranslateToChinese(v);
    ...
    html += `<div class="eng-table-wrap"><table><thead><tr>` +
        `<th>主语</th><th>谓语动词</th><th>宾语/表语</th><th>定语</th>` +
        `<th>状语（时间）</th><th>状语（地点）</th><th>状语（方式）</th><th>补足语</th>` +
        `</tr></thead><tbody>`;
    // 英文行
    html += `<tr>` +
        `<td>${cell(p.subject)}</td><td>${cell(p.verb)}</td>` +
        `<td>${cell(p.object)}</td><td>${cell(p.modifier)}</td>` +
        `<td>${cell(p.advTime)}</td><td>${cell(p.advPlace)}</td>` +
        `<td>${cell(p.advManner)}</td><td>${cell(p.complement)}</td>` +
        `</tr>`;
    // 中文行（按英文同列）
    html += `<tr>` +
        `<td>${zhCell(p.subject)}</td><td>${zhCell(p.verb)}</td>` +
        `<td>${zhCell(p.object)}</td><td>${zhCell(p.modifier)}</td>` +
        `<td>${zhCell(p.advTime)}</td><td>${zhCell(p.advPlace)}</td>` +
        `<td>${zhCell(p.advManner)}</td><td>${zhCell(p.complement)}</td>` +
        `</tr>`;
