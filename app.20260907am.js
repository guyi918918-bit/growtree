...
function renderBeautyToday() {
    ...
    const displayCity = mode === 'gps' ? (state.settings.gpsCityName || '📍 当前位置') : (city && city !== '__other__' ? city : (province && province !== '__other__' ? province + '（自定义）' : '请选择城市'));
    const w = cache.data || {};
    ...
    return `
        <div class="card-header" style="align-items:flex-start;gap:8px">
            ...
            <div class="beauty-city-bar" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;flex-shrink:0;justify-content:flex-end">
                <select id="beautyProvinceSelect" class="input" data-action="beauty-province-change" style="min-width:70px;width:auto;padding:5px 8px;font-size:12px;border-radius:8px;background:var(--card-bg);border:1px solid var(--border-color)">
                    ${beautyProvinceOptionsHTML(province)}
                </select>
                <select id="beautyCitySelect" class="input" data-action="beauty-city-change" style="min-width:80px;width:auto;padding:5px 8px;font-size:12px;border-radius:8px;background:var(--card-bg);border:1px solid var(--border-color)">
                    ${province ? beautyCityOptionsHTML(province, city) : '<option value="">请先选择省份</option>'}
                </select>
                <input type="text" id="beautyCityInput" class="input" placeholder="输入城市名" value="${escapeHtml(cityInput)}" style="display:${cityInputMode ? 'inline-block' : 'none'};width:80px;padding:5px 8px;font-size:12px;border-radius:8px;background:var(--card-bg);border:1px solid var(--border-color)">
                <button class="btn btn-secondary" data-action="beauty-city-confirm" style="font-size:12px;padding:5px 10px;white-space:nowrap;display:${cityInputMode ? 'inline-block' : 'none'}">确定</button>
...
function getBeautyPosition() {
    return new Promise(async (resolve) => {
        ...
        const c = (state.settings.city || '').trim();
        if (c) {
            const coords = await getCityCoordinates(c);
...
function initEvents() {
    ...
    document.addEventListener('click', async e => {
        ...
        if (action === 'beauty-province-change') {
            const sel = document.getElementById('beautyProvinceSelect');
            const citySel = document.getElementById('beautyCitySelect');
            ...
            if (sel) {
                ...
                if (pv && pv !== '__other__') {
                    ...
                    state.settings.province = pv;
                    state.settings.city = '';
                    if (citySel) {
                        citySel.style.display = 'inline-block';
                        citySel.innerHTML = beautyCityOptionsHTML(pv, '');
                    }
        ...
        if (action === 'beauty-city-change') {
            const sel = document.getElementById('beautyCitySelect');
            const input = document.getElementById('beautyCityInput');
            const confirm = document.querySelector('[data-action="beauty-city-confirm"]');
            if (sel && input) {
                if (sel.value === '__other__') {
                    ...
                    if (confirm) confirm.style.display = 'none';
                    state.settings.weatherMode = 'city';
                    state.settings.city = sel.value;
                    if (!state.settings.province) state.settings.province = beautyFindProvinceByCity(sel.value);
                    delete state.data.apiCache['beautyWeather'];
                    saveState();
                    if (typeof queueSync === 'function') queueSync();
                    loadBeautyWeather();
