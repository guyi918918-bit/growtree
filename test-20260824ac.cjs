...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824ac.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824ac.css'), 'utf8');

const BUILD = '20260824ac';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 95) ok('SW CACHE 已递增到 v95'); else no('SW CACHE 未递增');
