...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824y.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824y.css'), 'utf8');

const BUILD = '20260824y';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 91) ok('SW CACHE 已递增到 v91'); else no('SW CACHE 未递增');
...
try {
    ...
    const afterAllZero = run('state.data.points.history.filter(h => h.type === "base" && h.points !== 0).length');
    const afterCloudCheckIn = run('state.data.points.history.some(h => h.checkInId === "c1")');
    if (afterEarned === 0 && afterRankStars === 0 && afterBaseCount === 5 && afterAllZero === 0 && afterCloudCheckIn) {
        ok('清零后云端旧污染不回潮，双方 base 打卡记录均保留且积分已置 0（共 5 条，含云端 c1）');
    } else {
        no('清零后同步异常：' + JSON.stringify({ afterEarned, afterRankStars, afterBaseCount, afterAllZero, afterCloudCheckIn }));
    }
} catch (e) {
...
try {
    ...
    if (afterEarned === 0 && afterRankStars === 0 && afterBaseCount === 4 && afterAllZero === 0 && !!afterClearedAt) {
        ok('手机同步电脑(已清零) 后两端一致：4 条打卡保留、积分/段位归零');
    } else {
        no('手机/电脑同步异常：' + JSON.stringify({ afterEarned, afterRankStars, afterBaseCount, afterAllZero, afterClearedAt }));
    }
} catch (e) {
