...
function flRenderDiet() {
    ...
    const mealHtml = meals.length
        ? meals.map((meal, idx) => {
            ...
            const cal = flEstimateMealCalories(meal.items || []);
            return `<div class="fl-diet-item ${meal.status === 'done' ? 'completed' : ''} cat-${cat}" data-index="${idx}">
                <div class="fl-diet-main">
                    ...
                    <div class="fl-diet-name">${escapeHtml(meal.name)}</div>
                    ...
                    ${cal.total > 0 ? `<div class="fl-diet-cal">🔥 约 ${cal.total} 千卡（按100g估算）</div>` : ''}
                </div>
    ...
    <div class="fl-diet-page">
        <div class="card fl-diet-today-card">
            ...
            <div class="fl-diet-summary" id="flDietSummary"></div>
            <div class="fl-diet-list">${mealHtml}</div>
        </div>
        <div class="card fl-diet-search-card">
            <div class="fl-cal-search-bar">
...
function flRenderDietTodaySection() {
    const fl = flGetData();
    const budget = fl.user.calorieBudget || 1500;
    const consumed = flTodayCalories();
    ...
    <div class="fl-diet-today-section">
        <div class="fl-summary-card">
            <div class="fl-summary-row">
                <span>🔥 今日摄入</span>
                <span class="fl-summary-kcal">${consumed} <small>/ ${budget} 千卡</small></span>
            </div>
            <div class="fl-progress"><div class="fl-progress-fill ${pct > 100 ? 'over' : ''}" style="width:${pct}%"></div></div>
            <div class="fl-summary-row fl-summary-muted">
                <span>${remaining >= 0 ? `还可摄入 ${remaining} 千卡` : `已超标 ${Math.abs(remaining)} 千卡`}</span>
                <span>${pct}%</span>
            </div>
        </div>
...
function flRenderDietForm(index = -1) {
    ...
    const cat = meal && meal.category ? meal.category : '';
    const catOptions = [['早餐','早餐'],['午餐','午餐'],['晚餐','晚餐'],['加餐','加餐'],['其他','其他']];
    return `
    <div class="fl-form-overlay" id="flDietFormOverlay">
        <div class="fl-form-card">
            ...
            <div class="fl-form-row"><label>餐名</label><input type="text" id="flDietName" value="${meal ? escapeHtml(meal.name) : ''}" placeholder="例如：早餐"></div>
            <div class="fl-form-row"><label>分类</label><select id="flDietCategory">${catOptions.map(([v, l]) => `<option value="${v}" ${cat === v ? 'selected' : ''}>${l}</option>`).join('')}</select></div>
            <div class="fl-form-row"><label>食物（用逗号分隔）</label><textarea id="flDietItems" rows="2" placeholder="例如：水煮蛋 1个, 全麦面包 2片">${meal ? escapeHtml((meal.items || []).join(', ')) : ''}</textarea></div>
            <div class="fl-form-row"><label>状态</label><select id="flDietStatus"><option value="plan" ${!meal || meal.status === 'plan' ? 'selected' : ''}>计划中</option><option value="done" ${meal && meal.status === 'done' ? 'selected' : ''}>已打卡</option></select></div>
            ${!meal ? '<div class="fl-form-row fl-checkbox-row"><label class="fl-checkbox-label"><input type="checkbox" id="flDietSaveRecipe" checked> 同时保存到菜谱（下次可快速添加）</label></div>' : ''}
...
function flHandleDietSave() {
    ...
    const category = document.getElementById('flDietCategory').value;
...
function initEvents() {
    ...
    document.addEventListener('click', async e => {
        ...
        if (action === 'fl-recipe-online') {
            window.open('https://www.boohee.com/food/search?keyword=%E5%87%8F%E8%84%82%E9%A4%90', '_blank');
            toast('已在新标签页打开减脂菜谱推荐');
            return;
        }
