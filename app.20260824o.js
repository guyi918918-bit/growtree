    ...
    try {
        ...
        if (data && data.payload) {
            ...
            if (force || !state.data.lastSyncAt || (cloudUpdatedAt && cloudUpdatedAt > state.data.lastSyncAt)) {
                ...
                // 积分校准冲突解决：哪边校准版本更新，就以哪边的积分/段位/心愿/补打卡为准，
                // 防止污染数据通过 mergeById 又累加回来。
    ...
    if (stateTab === 'search') {
        ...
        if (!filtered.length) {
            ...
            itemsHtml = filtered.map((item) => {
                ...
                <div class="eng-history-detail" id="eng-search-detail-${realIndex}">搜索内容：${escapeHtml(item.query)}</div>`;
            }).join('');
        }
    } else {
