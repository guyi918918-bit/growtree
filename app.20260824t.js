...
function migrateData() {
    ...
    if (!fl.user || typeof fl.user !== 'object') fl.user = { name: '我的名字', gender: '女', age: 28, height: 163, startWeight: 120, targetWeight: 100, stepSize: 0.2, bodyFat: null, visceralFat: null, arm: null, waist: null, abdomen: null, hip: null, thigh: null, calf: null, note: '', motto: '💪 每完成一个小阶梯，都是向目标迈进的一大步！' };
...
function flRenderDiet() {
    ...
    const mealItems = meals.length
        ? meals.map((meal, idx) => `<div class="fl-diet-item ${meal.status === 'done' ? 'completed' : ''}">
            <div class="fl-diet-time">${escapeHtml(meal.time)}</div>
            <div class="fl-diet-content">
            ...
            <div class="fl-diet-actions">
                <button class="fl-diet-toggle" data-action="fl-diet-toggle" data-index="${idx}" title="切换状态">✔️</button>
        ...
        : '<p class="fl-empty">还没有饮食记录，点击上方添加</p>';
    return `
    <div class="fl-diet-page">
        <div class="card">
            ...
            <div class="fl-diet-list">${mealItems}</div>
        </div>
        <div class="card">
            <div class="fl-card-header"><div class="fl-card-title">🍳 减脂菜谱</div><div class="fl-recipe-filter"><button class="fl-filter-btn ${!state._flRecipeFilter || state._flRecipeFilter === 'all' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="all">全部</button><button class="fl-filter-btn ${state._flRecipeFilter === '早餐' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="早餐">早</button><button class="fl-filter-btn ${state._flRecipeFilter === '午餐' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="午餐">午</button><button class="fl-filter-btn ${state._flRecipeFilter === '晚餐' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="晚餐">晚</button><button class="fl-filter-btn ${state._flRecipeFilter === '加餐' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="加餐">加</button></div></div>
            <div class="fl-recipe-list" id="flRecipeList"></div>
            <button class="btn btn-secondary" data-action="fl-recipe-add" style="margin-top:10px;width:100%">➕ 新建菜谱</button>
        </div>
    </div>`;
}
...
function flHandleDietSave() {
    ...
    const meal = { time, name, items, status };
...
function flHandleRecipeUse(id) {
    ...
    if (!recipe) return;
    fl.diet.meals.push({ time: new Date().toTimeString().slice(0, 5), name: recipe.name, items: recipe.ingredients.slice(), status: 'plan', note: '📖 来自菜谱' });
    flSaveData();
    render();
