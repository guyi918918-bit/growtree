...
function migrateData() {
    ...
    // 健康减脂：阶梯减脂工作台数据
    if (!state.data.fatloss || typeof state.data.fatloss !== 'object') state.data.fatloss = {};
    const fl = state.data.fatloss;
    if (!fl.user || typeof fl.user !== 'object') fl.user = { name: '我的名字', gender: '女', age: 28, height: 163, startWeight: 120, targetWeight: 100, stepSize: 0.2, bodyFat: null, visceralFat: null, arm: null, waist: null, abdomen: null, hip: null, thigh: null, calf: null, note: '', motto: '💪 每完成一个小阶梯，都是向目标迈进的一大步！', calorieBudget: 1500 };
    if (typeof fl.user.calorieBudget !== 'number') fl.user.calorieBudget = 1500;
    if (!Array.isArray(fl.stepRecords)) fl.stepRecords = [];
    if (!Array.isArray(fl.circumferenceHistory)) fl.circumferenceHistory = [];
    if (!fl.diet || typeof fl.diet !== 'object') fl.diet = { meals: [], checkinEmojis: {} };
    if (!Array.isArray(fl.diet.meals)) fl.diet.meals = [];
    if (!fl.diet.checkinEmojis || typeof fl.diet.checkinEmojis !== 'object') fl.diet.checkinEmojis = {};
    if (!Array.isArray(fl.recipes)) fl.recipes = [
        { id: 1, name: '鸡胸肉沙拉', category: '午餐', ingredients: ['鸡胸肉', '生菜', '番茄', '黄瓜'] },
        { id: 2, name: '燕麦酸奶碗', category: '早餐', ingredients: ['燕麦', '酸奶', '蓝莓', '坚果'] },
        { id: 3, name: '清蒸鲈鱼', category: '晚餐', ingredients: ['鲈鱼', '姜', '葱', '蒸鱼豉油'] },
        { id: 4, name: '水果拼盘', category: '加餐', ingredients: ['苹果', '橙子', '猕猴桃'] }
    ];
    if (typeof fl._nextRecipeId !== 'number') fl._nextRecipeId = 5;
    if (!Array.isArray(fl.achievements)) fl.achievements = [
        { id: 'first', label: '首战告捷', icon: '🏅', unlocked: false },
        { id: '3steps', label: '连下三城', icon: '⭐', unlocked: false },
        { id: '5steps', label: '五阶勇士', icon: '🌟', unlocked: false },
        { id: '10steps', label: '十阶达人', icon: '🏆', unlocked: false },
        { id: 'halfway', label: '半程英雄', icon: '🌙', unlocked: false },
        { id: 'goal', label: '目标达成', icon: '🎯', unlocked: false },
        { id: 'streak3', label: '连续3天打卡', icon: '🔥', unlocked: false },
        { id: 'streak7', label: '连续7天打卡', icon: '💫', unlocked: false }
    ];
    try {
...
function render() {
    ...
    // 健康减脂模块需要在 DOM 渲染后初始化图表/日历/菜谱列表
    if (state.currentModule === 'fatloss') flBindAfterRender();
}
...
const moduleRenderers = {
    ...
    fatloss() {
        const tab = validSubTab('fatloss', FATLOSS_TABS, 'fl-overview');
        let html = `<div class="card tab-card">${subTabs(FATLOSS_TABS, tab, 'fatloss')}</div><div class="fl-page">${renderFatlossPage(tab)}</div>`;
        if (state._flShowDietForm) html += flRenderDietForm(state._flDietEditIndex || -1);
        if (state._flShowRecipeForm) html += flRenderRecipeForm(state._flRecipeEditId || -1);
        return html;
    },
...
// ==================== 健康减脂（阶梯减脂工作台 · 一级模块） ====================
// 说明：本模块参考“阶梯减脂工作台”实现，数据保存在 state.data.fatloss，
// 跟随 growtree 的 saveState / 云端同步机制，CSS 类统一 fl- 前缀，
// 不与其他模块共享数据/样式/事件。

const FATLOSS_TABS = [
    { id: 'fl-overview', name: '总览', icon: '📊' },
    { id: 'fl-diet', name: '饮食', icon: '🍽️' },
    { id: 'fl-profile', name: '档案', icon: '👤' },
    { id: 'fl-settings', name: '设置', icon: '⚙️' }
];

const FL_DIET_SUBTABS = [
    { id: 'today', name: '今日', icon: '📝' },
    { id: 'recipes', name: '菜谱', icon: '📖' }
];

// 常见食物热量（千卡/100g）本地库，用于离线估算与联网失败兜底
const FL_CALORIE_DB = {
    '米饭': 116, '糙米饭': 111, '燕麦': 389, '全麦面包': 247, '馒头': 223, '面条': 137,
    '红薯': 86, '玉米': 112, '土豆': 77, '紫薯': 133,
    '鸡蛋': 144, '水煮蛋': 144, '茶叶蛋': 144, '煎蛋': 209, '鸡胸肉': 133, '鸡腿肉': 181,
    '牛肉': 250, '瘦牛肉': 106, '猪肉': 271, '鱼肉': 120, '鲈鱼': 105, '虾': 93, '豆腐': 76,
    '牛奶': 54, '酸奶': 70, '豆浆': 16, '坚果': 607, '花生': 567, '核桃': 646,
    '苹果': 52, '香蕉': 89, '橙子': 47, '猕猴桃': 61, '番茄': 18, '黄瓜': 15, '生菜': 15,
    '西兰花': 34, '菠菜': 23, '胡萝卜': 41, '冬瓜': 12, '木耳': 265,
    '食用油': 884, '橄榄油': 884, '花生酱': 588, '蜂蜜': 304,
    '可乐': 42, '奶茶': 50, '咖啡': 2, '白开水': 0
};

const FL_DIET_TIPS = [
    '减脂不是少吃，而是吃得对。优先选择高蛋白、低脂肪、高纤维的食物。',
    '每餐先吃蔬菜，再吃蛋白质，最后吃主食，有助于平稳血糖。',
    '喝水可以提高代谢，餐前喝一杯水能减少过量进食。',
    '少食多餐≠减肥，控制总热量才是关键。',
    '加工食品往往隐藏大量糖和油，尽量少吃包装零食。',
    '睡眠不足会影响瘦素分泌，规律作息也是减脂的一部分。'
];

// 联网推荐：由专业减脂场景 curated 的菜品库，按餐分类，可直接加入今日饮食或菜谱
const FL_ONLINE_RECIPES = {
    '早餐': [
        { name: '燕麦酸奶碗', items: ['燕麦 50g', '无糖酸奶 150g', '蓝莓 30g', '坚果 10g'], kcal: 320, tag: '高纤维' },
        { name: '全麦鸡蛋三明治', items: ['全麦面包 2片', '鸡蛋 1个', '生菜 2片', '番茄 2片'], kcal: 350, tag: '高蛋白' },
        { name: '紫薯豆浆餐', items: ['紫薯 1个', '无糖豆浆 250ml', '水煮蛋 1个'], kcal: 280, tag: '低脂' },
        { name: '牛油果滑蛋吐司', items: ['全麦吐司 1片', '鸡蛋 1个', '牛油果 1/4个'], kcal: 380, tag: '优质脂肪' },
        { name: '小米粥配小菜', items: ['小米粥 1碗', '黄瓜 1根', '茶叶蛋 1个'], kcal: 260, tag: '清淡' }
    ],
    '午餐': [
        { name: '鸡胸肉沙拉', items: ['鸡胸肉 100g', '生菜 50g', '番茄 50g', '黄瓜 50g', '橄榄油 5g'], kcal: 420, tag: '高蛋白' },
        { name: '清蒸鲈鱼配杂粮饭', items: ['鲈鱼 150g', '杂粮饭 100g', '西兰花 100g'], kcal: 450, tag: '低脂' },
        { name: '番茄牛肉意面', items: ['牛肉 80g', '全麦意面 80g', '番茄 100g', '洋葱 30g'], kcal: 520, tag: '饱腹' },
        { name: '黑椒虾仁便当', items: ['虾仁 120g', '糙米饭 100g', '胡萝卜 50g', '豌豆 30g'], kcal: 480, tag: '高蛋白' },
        { name: '豆腐蔬菜锅', items: ['嫩豆腐 150g', '白菜 100g', '香菇 50g', '海带 30g'], kcal: 320, tag: '素食' }
    ],
    '晚餐': [
        { name: '虾仁炒时蔬', items: ['虾仁 100g', '玉米 50g', '彩椒 50g', '西兰花 80g'], kcal: 360, tag: '低脂' },
        { name: '紫菜蛋花汤配玉米', items: ['玉米 1根', '紫菜蛋花汤 1碗', '凉拌黄瓜 1份'], kcal: 280, tag: '清淡' },
        { name: '香煎鸡胸肉配芦笋', items: ['鸡胸肉 100g', '芦笋 100g', '口蘑 50g'], kcal: 380, tag: '高蛋白' },
        { name: '凉拌鸡丝荞麦面', items: ['荞麦面 80g', '鸡胸肉 80g', '黄瓜丝 50g', '胡萝卜丝 30g'], kcal: 420, tag: '饱腹' },
        { name: '冬瓜蛤蜊汤', items: ['蛤蜊 100g', '冬瓜 150g', '姜丝 5g'], kcal: 180, tag: '低卡' }
    ],
    '加餐': [
        { name: '水果拼盘', items: ['苹果 1个', '橙子 1个', '猕猴桃 1个'], kcal: 180, tag: '维生素' },
        { name: '希腊酸奶杯', items: ['希腊酸奶 100g', '奇亚籽 5g', '草莓 3颗'], kcal: 150, tag: '蛋白质' },
        { name: '坚果小把', items: ['巴旦木 10g', '核桃 10g'], kcal: 120, tag: '优质脂肪' },
        { name: '黄瓜条蘸鹰嘴豆泥', items: ['黄瓜 1根', '鹰嘴豆泥 30g'], kcal: 110, tag: '低卡' },
        { name: '无糖豆浆', items: ['无糖豆浆 250ml'], kcal: 80, tag: '植物蛋白' }
    ]
};

function renderFatlossPage(tab) {
    if (tab === 'fl-overview') return flRenderOverview();
    if (tab === 'fl-diet') return flRenderDiet();
    if (tab === 'fl-profile') return flRenderProfile();
    if (tab === 'fl-settings') return flRenderSettings();
    return flRenderOverview();
}

function flGetData() {
    return state.data.fatloss;
}
...
function flGenerateSteps(start, target, stepSize) {
    if (start == null || target == null || !stepSize || stepSize <= 0) return [];
    const steps = [];
    const direction = start > target ? -1 : 1;
    const total = Math.abs(start - target);
    const count = Math.max(1, Math.round(total / stepSize));
    for (let i = 0; i < count; i++) {
        const val = start + direction * i * stepSize;
        if (direction > 0 && val > target) break;
        if (direction < 0 && val < target) break;
        steps.push(parseFloat(val.toFixed(2)));
    }
...
function flGetStepStatus(steps, records) {
    const recordMap = {};
    (records || []).forEach(r => { recordMap[r.target] = r; });
    return steps.map((target, i) => {
        if (recordMap[target]) return { target, status: 'completed', record: recordMap[target] };
        let allPrevDone = true;
        for (let j = 0; j < i; j++) {
            if (!recordMap[steps[j]]) { allPrevDone = false; break; }
        }
        return { target, status: allPrevDone ? 'active' : 'pending', record: null };
    });
}

function flGetCurrentStep(steps, records) {
    const status = flGetStepStatus(steps, records);
    for (const s of status) {
        if (s.status === 'active') return s;
    }
    if (status.length && status[status.length - 1].status === 'completed') {
        return { target: status[status.length - 1].target, status: 'done', record: status[status.length - 1].record };
    }
    return null;
}

function flGetCompletedCount(steps, records) {
    const recordMap = {};
    (records || []).forEach(r => { recordMap[r.target] = r; });
    return steps.filter(t => recordMap[t]).length;
}

function flGetCurrentWeight(records) {
    if (!records || !records.length) return null;
    const sorted = [...records].sort((a, b) => (a.completedAt > b.completedAt ? -1 : 1));
    return sorted[0].weight;
}

function flGetTotalLost(startWeight, records) {
    const current = flGetCurrentWeight(records);
    if (current == null || startWeight == null) return null;
    return parseFloat((startWeight - current).toFixed(2));
}

function flGetStreak(records) {
    if (!records.length) return 0;
    const sorted = [...records].sort((a, b) => (a.completedAt > b.completedAt ? -1 : 1));
    const dates = sorted.map(r => r.completedAt.slice(0, 10));
    const today = flTodayStr();
    let streak = 0;
...
function flUpdateAchievements() {
    const fl = flGetData();
    const u = fl.user;
    const steps = flGenerateSteps(u.startWeight, u.targetWeight, u.stepSize);
    const completed = flGetCompletedCount(steps, fl.stepRecords);
    const total = steps.length;
    const streak = flGetStreak(fl.stepRecords);
    const map = { first: completed >= 1, '3steps': completed >= 3, '5steps': completed >= 5, '10steps': completed >= 10, halfway: total > 0 && completed >= Math.floor(total / 2), goal: total > 0 && completed >= total, streak3: streak >= 3, streak7: streak >= 7 };
    fl.achievements.forEach(a => { if (map[a.id]) a.unlocked = true; });
}

function flGetCollapsed() {
    if (!state._flCollapsed) state._flCollapsed = { history: false, steps: true };
    return state._flCollapsed;
}
function flIsCollapsed(section) {
    return !!flGetCollapsed()[section];
}

function flRenderOverview() {
    const fl = flGetData();
    const u = fl.user;
    const steps = flGenerateSteps(u.startWeight, u.targetWeight, u.stepSize);
    const records = fl.stepRecords || [];
    const statusList = flGetStepStatus(steps, records);
    const current = flGetCurrentStep(steps, records);
    const completedCount = flGetCompletedCount(steps, records);
    const totalSteps = steps.length;
    const currentWeight = flGetCurrentWeight(records);
    const totalLost = flGetTotalLost(u.startWeight, records);
    const streak = flGetStreak(records);
    let stepCardHtml = '';
    if (current && current.status === 'active') {
        const progress = totalSteps > 0 ? Math.min(100, Math.round(completedCount / totalSteps * 100)) : 0;
        stepCardHtml = `<div class="fl-step-card active"><div class="fl-step-header"><span>🎯 当前目标</span><span>${completedCount} / ${totalSteps}</span></div><div class="fl-step-target">${current.target.toFixed(1)} <small>斤</small></div><div class="fl-progress"><div class="fl-progress-fill" style="width:${progress}%"></div></div><div class="fl-step-footer"><span>⏳ 进行中 · 加油！</span><span>${progress}%</span></div></div>`;
    } else if (current && current.status === 'done') {
        stepCardHtml = `<div class="fl-step-card done"><div class="fl-step-header"><span>🎉 目标达成！</span><span>${completedCount} / ${totalSteps}</span></div><div class="fl-step-target">${current.target.toFixed(1)} <small>斤</small></div><div class="fl-progress"><div class="fl-progress-fill" style="width:100%"></div></div><div class="fl-step-footer"><span>🏆 已达成目标体重</span><span>100%</span></div></div>`;
    } else {
        stepCardHtml = `<div class="fl-step-card"><div class="fl-step-header"><span>🎯 等待开始</span><span>0 / ${totalSteps || 0}</span></div><div class="fl-step-target">-- <small>斤</small></div><div class="fl-progress"><div class="fl-progress-fill" style="width:0%"></div></div><div class="fl-step-footer"><span>请设置起始和目标体重</span><span>0%</span></div></div>`;
    }
    const historyItems = records.length
        ? [...records].sort((a, b) => (a.completedAt > b.completedAt ? -1 : 1)).map(r => `<div class="fl-history-item"><span class="fl-time">${flFormatTime(r.completedAt)}</span><span class="fl-weight">${r.weight.toFixed(1)} 斤</span><span class="fl-step-target">目标 ${r.target.toFixed(1)} 斤</span><span class="fl-badge">✅ 完成</span></div>`).join('')
        : '<p class="fl-empty">还没有打卡记录，开始你的第一个阶梯吧！</p>';
    const stepItems = statusList.length
        ? statusList.map((item, idx) => {
            const icon = item.status === 'completed' ? '✅' : item.status === 'active' ? '▶️' : '⬜';
            const label = item.status === 'completed' ? flFormatTime(item.record.completedAt) : item.status === 'active' ? '进行中' : '待完成';
            return `<div class="fl-step-row ${item.status}"><span class="fl-idx">${idx + 1}</span><span class="fl-target">${item.target.toFixed(1)} 斤</span><span class="fl-status-icon">${icon}</span><span class="fl-status-label">${label}</span></div>`;
        }).join('')
        : '<p class="fl-empty">请设置起始和目标体重</p>';
    return `
    <div class="fl-overview">
        <div class="card">
            <div class="fl-user-row">
                <div class="fl-avatar">${u.gender === '男' ? '👨' : '👩'}</div>
                <div class="fl-user-info">
                    <h2>${escapeHtml(u.name || '我的名字')} <span>(减脂版)</span></h2>
                    <div class="fl-meta">
                        <span>起始 <strong>${u.startWeight != null ? u.startWeight.toFixed(1) : '--'}</strong> 斤</span>
                        <span>目标 <strong>${u.targetWeight != null ? u.targetWeight.toFixed(1) : '--'}</strong> 斤</span>
                        <span>已减 <strong class="fl-lost">${totalLost != null ? totalLost.toFixed(1) : '--'}</strong> 斤</span>
                        <span>打卡 <strong>${records.length}</strong> 次</span>
                        ${streak > 0 ? `<span>连续 <strong>🔥${streak}</strong> 天</span>` : ''}
                    </div>
                </div>
            </div>
            ${stepCardHtml}
            <div class="fl-today-weight">
                <label>📌 今日体重</label>
                <input type="number" id="flTodayWeight" step="0.1" placeholder="斤" value="${currentWeight != null ? currentWeight : ''}">
                <button class="btn btn-primary" data-action="fl-checkin">打卡</button>
            </div>
            <div class="fl-section ${flIsCollapsed('history') ? 'collapsed' : ''}" data-section="history">
                <div class="fl-section-title fl-collapsible" data-action="fl-toggle-section" data-section="history"><span>📋 打卡记录</span><span class="fl-section-meta"><span>完成时间 · 体重</span><span class="fl-collapse-icon">▾</span></span></div>
                <div class="fl-history-list">${historyItems}</div>
            </div>
            <div class="fl-section ${flIsCollapsed('steps') ? 'collapsed' : ''}" data-section="steps">
                <div class="fl-section-title fl-collapsible" data-action="fl-toggle-section" data-section="steps"><span>📐 阶梯进度</span><span class="fl-section-meta"><span>${totalSteps} 个阶梯</span><span class="fl-collapse-icon">▾</span></span></div>
                <div class="fl-step-list">${stepItems}</div>
            </div>
            <div class="fl-motto"><span id="flMottoText">${escapeHtml(u.motto || '💪 每完成一个小阶梯，都是向目标迈进的一大步！')}</span><span class="fl-refresh" data-action="fl-refresh-motto">🔄</span></div>
        </div>
    </div>`;
}

function flRenderDiet() {
    const fl = flGetData();
    const meals = fl.diet.meals || [];
    const subTab = state._flDietSubTab || 'today';
    const mealHtml = meals.length
        ? meals.map((meal, idx) => {
            const cat = flMealCategory(meal);
            return `<div class="fl-diet-item ${meal.status === 'done' ? 'completed' : ''} cat-${cat}" data-index="${idx}">
                <div class="fl-diet-main">
                    <div class="fl-diet-title"><span class="fl-diet-time">${escapeHtml(meal.time)}</span> <span class="fl-diet-cat">${escapeHtml(meal.name || cat)}</span></div>
                    <div class="fl-diet-items">${(meal.items || []).map(i => `<span class="fl-tag">${escapeHtml(i)}</span>`).join('')}</div>
                    ${meal.note ? `<div class="fl-diet-note">${escapeHtml(meal.note)}</div>` : ''}
                </div>
                <div class="fl-diet-right">
                    <span class="fl-diet-badge ${meal.status}">${meal.status === 'done' ? '✅ 已打卡' : meal.status === 'plan' ? '⏳ 计划中' : '❌ 未打卡'}</span>
                    <div class="fl-diet-actions">
                        <button class="fl-diet-toggle" data-action="fl-diet-toggle" data-index="${idx}" title="切换状态">✓</button>
                        <button class="fl-diet-edit" data-action="fl-diet-edit" data-index="${idx}" title="编辑">✎</button>
                        <button class="fl-diet-delete" data-action="fl-diet-delete" data-index="${idx}" title="删除">✕</button>
                    </div>
                </div>
            </div>`;
        }).join('')
        : '<p class="fl-empty">还没有饮食记录，点击上方“添加”记录每一餐</p>';
    return `
    <div class="fl-diet-page">
        <div class="card fl-diet-today-card">
            <div class="fl-card-header"><div class="fl-card-title">🍽 今日饮食</div><button class="fl-card-action" data-action="fl-diet-add">➕ 添加</button></div>
            <div class="fl-diet-list">${mealHtml}</div>
        </div>
        <div class="card fl-diet-tabs-card">
            <div class="fl-diet-subtabs">
                ${FL_DIET_SUBTABS.map(t => `<button class="fl-diet-subtab ${subTab === t.id ? 'active' : ''}" data-action="fl-diet-subtab" data-subtab="${t.id}"><span>${t.icon}</span>${t.name}</button>`).join('')}
            </div>
            ${subTab === 'recipes' ? flRenderDietRecipesSection() : flRenderDietTodaySection()}
        </div>
    </div>`;
}

function flRenderDietTodaySection() {
    const tip = FL_DIET_TIPS[Math.floor(Math.random() * FL_DIET_TIPS.length)];
    return `
    <div class="fl-diet-today-section">
        <div class="fl-coach-tip"><span>💡</span><p>${tip}</p></div>
        <div class="fl-cal-search-bar" style="margin-top:12px">
            <input type="text" id="flCalInput" placeholder="输入食物名称查热量" value="${escapeHtml(state._flCalQuery || '')}">
            <select id="flCalEngine" data-action="fl-cal-engine-change">
                <option value="online" ${state._flCalEngine !== 'baidu' ? 'selected' : ''}>在线热量库</option>
                <option value="baidu" ${state._flCalEngine === 'baidu' ? 'selected' : ''}>百度</option>
            </select>
            <button class="btn btn-primary" data-action="fl-cal-search">🔍 搜索</button>
        </div>
        <div class="fl-cal-results" id="flCalResults"></div>
    </div>`;
}

function flRenderDietRecipesSection() {
    const filter = state._flRecipeFilter || 'all';
    const onlinePanel = state._flShowOnlineRecipes ? flRenderOnlineRecipesModal() : '';
    return `
    <div class="fl-diet-recipe-section">
        <div class="fl-recipe-toolbar">
            <button class="btn btn-primary" data-action="fl-recipe-add">➕ 新建菜谱</button>
            <button class="btn btn-secondary" data-action="fl-recipe-online">🌐 联网推荐</button>
        </div>
        ${onlinePanel}
        <div class="fl-recipe-filter">
            <button class="fl-filter-btn ${filter === 'all' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="all">全部</button>
            <button class="fl-filter-btn ${filter === '早餐' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="早餐">早餐</button>
            <button class="fl-filter-btn ${filter === '午餐' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="午餐">午餐</button>
            <button class="fl-filter-btn ${filter === '晚餐' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="晚餐">晚餐</button>
            <button class="fl-filter-btn ${filter === '加餐' ? 'active' : ''}" data-action="fl-recipe-filter" data-cat="加餐">加餐</button>
        </div>
        <div class="fl-recipe-list" id="flRecipeList"></div>
    </div>`;
}

function flRenderOnlineRecipesModal() {
    const cat = state._flOnlineRecipeCat || '早餐';
    const cats = ['早餐', '午餐', '晚餐', '加餐'];
    const list = FL_ONLINE_RECIPES[cat] || [];
    return `
    <div class="fl-form-overlay" id="flOnlineRecipesOverlay" style="z-index:1100">
        <div class="fl-form-card fl-online-recipes-card">
            <div class="fl-form-header"><span>🌐 联网推荐菜品</span><button data-action="fl-online-recipes-close">✕</button></div>
            <div class="fl-online-cats">
                ${cats.map(c => `<button class="fl-filter-btn ${cat === c ? 'active' : ''}" data-action="fl-online-recipe-cat" data-cat="${c}">${c}</button>`).join('')}
            </div>
            <div class="fl-online-list">
                ${list.map((r, i) => `<div class="fl-online-item">
                    <div class="fl-online-info">
                        <div class="fl-online-name">${escapeHtml(r.name)} <span class="fl-online-tag">${escapeHtml(r.tag)}</span></div>
                        <div class="fl-online-items">${r.items.map(it => `<span class="fl-tag">${escapeHtml(it)}</span>`).join('')}</div>
                        <div class="fl-online-kcal">🔥 约 ${r.kcal} 千卡</div>
                    </div>
                    <div class="fl-online-actions">
                        <button class="btn btn-sm btn-primary" data-action="fl-online-add" data-cat="${escapeHtml(cat)}" data-idx="${i}">➕ 加入今日</button>
                        <button class="btn btn-sm btn-secondary" data-action="fl-online-save" data-cat="${escapeHtml(cat)}" data-idx="${i}">📖 存菜谱</button>
                    </div>
                </div>`).join('')}
            </div>
            <div class="fl-form-actions"><button class="btn btn-secondary" data-action="fl-online-recipes-close">关闭</button></div>
        </div>
    </div>`;
}

function flMealCategory(meal) {
    if (meal && meal.category) return meal.category;
    const name = (meal && meal.name) || '';
    if (/早/.test(name)) return '早餐';
    if (/午|中/.test(name)) return '午餐';
    if (/晚/.test(name)) return '晚餐';
    if (/加|零食|点心|水果|坚果/.test(name)) return '加餐';
    return '其他';
}

function flEstimateMealCalories(items) {
    let total = 0, known = 0;
    (items || []).forEach(it => {
        const text = String(it).trim();
        if (!text) return;
        // 优先精确匹配，再模糊匹配
        let k = FL_CALORIE_DB[text];
        if (k == null) {
            const found = Object.keys(FL_CALORIE_DB).find(name => text.includes(name));
            if (found) { k = FL_CALORIE_DB[found]; }
        }
        if (k != null) { total += k; known++; }
    });
    return { total: Math.round(total), known };
}

function flTodayCalories() {
    const meals = flGetData().diet.meals || [];
    return meals.reduce((sum, m) => sum + (flEstimateMealCalories(m.items || []).total || 0), 0);
}

function flRenderDietSummary() {
    const wrap = document.getElementById('flDietSummary');
    if (!wrap) return;
    wrap.innerHTML = flRenderDietTodaySection();
}

function flRenderCalResults() {
    const wrap = document.getElementById('flCalResults');
    if (!wrap) return;
    if (!state._flCalResults) { wrap.innerHTML = ''; return; }
    if (state._flCalResults.loading) { wrap.innerHTML = '<p class="fl-cal-loading">正在查询…</p>'; return; }
    const list = state._flCalResults.results || [];
    if (!list.length) {
        wrap.innerHTML = `<p class="fl-cal-empty">未找到“${escapeHtml(state._flCalQuery || '')}”的热量数据，试试更常见的名称</p>`;
        return;
    }
    wrap.innerHTML = `<div class="fl-cal-results-title">找到 ${list.length} 条结果 <small>（${escapeHtml(state._flCalResults.source || '')}）</small></div>
        <div class="fl-cal-result-list">${list.map((r, i) => `<div class="fl-cal-result-item">
            <div class="fl-cal-result-info">
                <div class="fl-cal-result-name">${escapeHtml(r.name)}</div>
                <div class="fl-cal-result-kcal">${r.kcal} 千卡 / ${escapeHtml(r.serving || '100g')}</div>
            </div>
            <button class="btn btn-sm btn-primary" data-action="fl-cal-add" data-name="${escapeHtml(r.name)}">➕ 添加</button>
        </div>`).join('')}</div>`;
}

async function flSearchCaloriesOnline(q, engine) {
    if (engine === 'baidu') {
        window.open(`https://www.baidu.com/s?wd=${encodeURIComponent(q + ' 热量')}`, '_blank');
        return { source: '百度', results: [] };
    }
    try {
        const url = `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(q)}&json=1&page_size=6`;
        const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
        const data = await res.json();
        const results = (data.products || []).map(p => {
            const name = p.product_name || p.product_name_en || q;
            const nut = p.nutriments || {};
            const kcal = nut['energy-kcal_100g'] || nut['energy-kcal'] || null;
            const serving = p.serving_size || '100g';
            return { name, kcal, serving, source: 'OpenFoodFacts' };
        }).filter(r => r.kcal != null);
        if (results.length) return { source: 'OpenFoodFacts', results };
        throw new Error('no results');
    } catch (e) {
        // 兜底本地库
        const results = [];
        const query = q.trim();
        Object.entries(FL_CALORIE_DB).forEach(([name, kcal]) => {
            if (name.includes(query) || query.includes(name)) results.push({ name, kcal, serving: '100g', source: '本地库' });
        });
        return { source: '本地库', results: results.slice(0, 6) };
    }
}

function flHandleCalSearch() {
    const input = document.getElementById('flCalInput');
    const sel = document.getElementById('flCalEngine');
    const q = input ? input.value.trim() : '';
    const engine = sel ? sel.value : 'online';
    if (!q) { toast('请输入食物名称'); return; }
    state._flCalQuery = q;
    state._flCalEngine = engine;
    state._flCalResults = { loading: true };
    flRenderCalResults();
    flSearchCaloriesOnline(q, engine).then(res => {
        state._flCalResults = { ...res, query: q };
        flRenderCalResults();
    });
}

function flHandleCalAdd(name) {
    if (!name) return;
    const fl = flGetData();
    const time = new Date().toTimeString().slice(0, 5);
    fl.diet.meals.push({ time, name: '加餐', items: [name], status: 'plan', category: '加餐', note: '🔍 来自热量查询' });
    flSaveData();
    render();
    toast(`已添加 “${name}” 到今日饮食`);
}
function flRenderProfile() {
    const fl = flGetData();
    const u = fl.user;
    const circ = fl.circumferenceHistory && fl.circumferenceHistory.length ? fl.circumferenceHistory[fl.circumferenceHistory.length - 1] : null;
    return `
    <div class="fl-profile-page">
        <div class="card">
            <div class="fl-card-header"><div class="fl-card-title">👤 基本信息</div></div>
            <div class="fl-form">
                <div class="fl-form-row"><label>昵称</label><input type="text" id="flProfileName" value="${escapeHtml(u.name || '')}"></div>
                <div class="fl-form-row"><label>性别</label><select id="flProfileGender"><option value="女" ${u.gender === '女' ? 'selected' : ''}>女</option><option value="男" ${u.gender === '男' ? 'selected' : ''}>男</option></select></div>
                <div class="fl-form-row"><label>年龄</label><input type="number" id="flProfileAge" value="${u.age || ''}"></div>
                <div class="fl-form-row"><label>身高 (cm)</label><input type="number" id="flProfileHeight" value="${u.height || ''}"></div>
                <div class="fl-form-row"><label>起始体重 (斤)</label><input type="number" id="flProfileStart" step="0.1" value="${u.startWeight != null ? u.startWeight : ''}"></div>
                <div class="fl-form-row"><label>目标体重 (斤)</label><input type="number" id="flProfileTarget" step="0.1" value="${u.targetWeight != null ? u.targetWeight : ''}"></div>
                <div class="fl-form-row"><label>阶梯步长 (斤)</label><input type="number" id="flProfileStepSize" step="0.1" value="${u.stepSize != null ? u.stepSize : 0.2}"></div>
                <div class="fl-form-row"><label>体脂率 (%)</label><input type="number" id="flProfileBodyFat" step="0.1" value="${u.bodyFat != null ? u.bodyFat : ''}"></div>
                <div class="fl-form-row"><label>内脏脂肪等级</label><input type="number" id="flProfileVisceral" step="0.1" value="${u.visceralFat != null ? u.visceralFat : ''}"></div>
                <div class="fl-form-row"><label>每日热量目标 (千卡)</label><input type="number" id="flProfileCalorieBudget" step="10" value="${u.calorieBudget != null ? u.calorieBudget : 1500}"></div>
                <div class="fl-form-row"><label>个性签名 / 目标宣言</label><textarea id="flProfileNote" rows="2">${escapeHtml(u.note || '')}</textarea></div>
                <button class="btn btn-primary" data-action="fl-save-profile" style="width:100%;margin-top:6px">💾 保存档案</button>
            </div>
        </div>
        <div class="card">
            <div class="fl-card-header"><div class="fl-card-title">📐 最新围度</div><span class="fl-text-muted">${circ ? flFormatTime(circ.date) : '未记录'}</span></div>
            <div class="fl-form">
                <div class="fl-form-row"><label>上臂 (cm)</label><input type="number" id="flCircArm" step="0.1" value="${circ ? circ.arm : (u.arm != null ? u.arm : '')}"></div>
                <div class="fl-form-row"><label>腰围 (cm)</label><input type="number" id="flCircWaist" step="0.1" value="${circ ? circ.waist : (u.waist != null ? u.waist : '')}"></div>
                <div class="fl-form-row"><label>腹围 (cm)</label><input type="number" id="flCircAbdomen" step="0.1" value="${circ ? circ.abdomen : (u.abdomen != null ? u.abdomen : '')}"></div>
                <div class="fl-form-row"><label>臀围 (cm)</label><input type="number" id="flCircHip" step="0.1" value="${circ ? circ.hip : (u.hip != null ? u.hip : '')}"></div>
                <div class="fl-form-row"><label>大腿 (cm)</label><input type="number" id="flCircThigh" step="0.1" value="${circ ? circ.thigh : (u.thigh != null ? u.thigh : '')}"></div>
                <div class="fl-form-row"><label>小腿 (cm)</label><input type="number" id="flCircCalf" step="0.1" value="${circ ? circ.calf : (u.calf != null ? u.calf : '')}"></div>
                <button class="btn btn-primary" data-action="fl-save-circ" style="width:100%;margin-top:6px">💾 记录围度</button>
            </div>
            <div class="fl-circ-history-wrap" id="flCircHistoryWrap"></div>
        </div>
    </div>`;
}

function flRenderSettings() {
    const fl = flGetData();
    const u = fl.user;
    const today = flTodayStr();
    const emojis = fl.diet.checkinEmojis || {};
    const todayEmoji = emojis[today] || '';
    const ach = (fl.achievements || []).map(a => `<div class="fl-achieve ${a.unlocked ? 'unlocked' : ''}"><span>${a.icon}</span><div>${a.label}</div></div>`).join('');
    return `
    <div class="fl-settings-page">
        <div class="card">
            <div class="fl-card-header"><div class="fl-card-title">🏅 成就墙</div></div>
            <div class="fl-achievements">${ach}</div>
        </div>
        <div class="card">
            <div class="fl-card-header"><div class="fl-card-title">📅 运动打卡日历</div><div class="fl-today-emoji">${todayEmoji ? `今日：${todayEmoji}` : '今天还没打卡哦'}</div></div>
            <div class="fl-checkin-calendar" id="flCheckinCalendar"></div>
            <div class="fl-emoji-picker" id="flEmojiPicker"></div>
        </div>
        <div class="card">
            <div class="fl-card-header"><div class="fl-card-title">⚙️ 减脂设置</div></div>
            <div class="fl-form">
                <div class="fl-form-row"><label>每日激励语</label><textarea id="flSettingsMotto" rows="2">${escapeHtml(u.motto || '')}</textarea></div>
                <button class="btn btn-primary" data-action="fl-save-motto" style="width:100%;margin-top:6px">💾 保存激励语</button>
            </div>
        </div>
        <div class="card">
            <div class="fl-card-header"><div class="fl-card-title">🧹 数据管理</div></div>
            <div style="display:flex;gap:8px;flex-wrap:wrap">
                <button class="btn btn-secondary" data-action="fl-reset-checkin">清空打卡记录</button>
                <button class="btn btn-secondary" data-action="fl-reset-diet">清空饮食记录</button>
                <button class="btn btn-danger" data-action="fl-reset-all">重置全部减脂数据</button>
            </div>
        </div>
    </div>`;
}

function flRenderDietForm(index = -1) {
    const fl = flGetData();
    const meal = index >= 0 && fl.diet.meals[index] ? fl.diet.meals[index] : null;
    return `
    <div class="fl-form-overlay" id="flDietFormOverlay">
        <div class="fl-form-card">
            <div class="fl-form-header"><span>${meal ? '编辑饮食' : '添加饮食'}</span><button data-action="fl-diet-form-close">✕</button></div>
            <input type="hidden" id="flDietEditIndex" value="${index}">
            <div class="fl-form-row"><label>时间</label><input type="time" id="flDietTime" value="${meal ? meal.time : '08:00'}"></div>
            <div class="fl-form-row"><label>餐名</label><input type="text" id="flDietName" value="${meal ? escapeHtml(meal.name) : ''}" placeholder="例如：早餐、午餐、晚餐"></div>
            <div class="fl-form-row"><label>食物（逗号分隔）</label><textarea id="flDietItems" rows="2" placeholder="例如：水煮蛋 1个, 全麦面包 2片">${meal ? escapeHtml((meal.items || []).join(', ')) : ''}</textarea></div>
            <div class="fl-form-row"><label>打卡</label><select id="flDietStatus"><option value="plan" ${!meal || meal.status === 'plan' ? 'selected' : ''}>计划中</option><option value="done" ${meal && meal.status === 'done' ? 'selected' : ''}>已打卡</option></select></div>
            ${!meal ? '<div class="fl-form-row fl-checkbox-row"><label class="fl-checkbox-label"><input type="checkbox" id="flDietSaveRecipe" checked> 同时保存到菜谱</label></div>' : ''}
            <div class="fl-form-actions"><button class="btn btn-secondary" data-action="fl-diet-form-close">取消</button><button class="btn btn-primary" data-action="fl-diet-form-save">${meal ? '更新' : '添加'}</button></div>
        </div>
    </div>`;
}

function flRenderRecipeForm(id = -1) {
    const fl = flGetData();
    const recipe = id >= 0 ? fl.recipes.find(r => r.id === id) : null;
    return `
    <div class="fl-form-overlay" id="flRecipeFormOverlay">
        <div class="fl-form-card">
            <div class="fl-form-header"><span>${recipe ? '编辑菜谱' : '新建菜谱'}</span><button data-action="fl-recipe-form-close">✕</button></div>
            <input type="hidden" id="flRecipeEditId" value="${id}">
            <div class="fl-form-row"><label>菜谱名</label><input type="text" id="flRecipeName" value="${recipe ? escapeHtml(recipe.name) : ''}" placeholder="例如：鸡胸肉沙拉"></div>
            <div class="fl-form-row"><label>分类</label><select id="flRecipeCategory"><option value="早餐" ${recipe && recipe.category === '早餐' ? 'selected' : ''}>早餐</option><option value="午餐" ${recipe && recipe.category === '午餐' ? 'selected' : ''}>午餐</option><option value="晚餐" ${recipe && recipe.category === '晚餐' ? 'selected' : ''}>晚餐</option><option value="加餐" ${recipe && recipe.category === '加餐' ? 'selected' : ''}>加餐</option></select></div>
            <div class="fl-form-row"><label>食材（用逗号分隔）</label><textarea id="flRecipeIngredients" rows="2" placeholder="例如：鸡胸肉, 生菜, 番茄">${recipe ? escapeHtml((recipe.ingredients || []).join(', ')) : ''}</textarea></div>
            <div class="fl-form-actions"><button class="btn btn-secondary" data-action="fl-recipe-form-close">取消</button><button class="btn btn-primary" data-action="fl-recipe-form-save">${recipe ? '更新' : '创建'}</button></div>
        </div>
    </div>`;
}

function flBindAfterRender() {
    const module = getModuleById(state.currentModule);
    if (!module || module.id !== 'fatloss') return;
    const tab = validSubTab('fatloss', FATLOSS_TABS, 'fl-overview');
    if (tab === 'fl-diet') {
        flRenderCalResults();
        flRenderRecipeList();
    ...
    if (tab === 'fl-settings') {
        flRenderCheckinCalendar();
    }
}

function flRenderCheckinCalendar() {
    const container = document.getElementById('flCheckinCalendar');
    const picker = document.getElementById('flEmojiPicker');
    if (!container || !picker) return;
    container.innerHTML = '';
    const fl = flGetData();
    const emojis = fl.diet.checkinEmojis || {};
    const todayStr = flTodayStr();
    ...
    wrap.className = 'fl-cal-wrap';
    wrap.innerHTML = `<div class="fl-cal-month">${monthLabel}</div><div class="fl-cal-weekdays">${weekdays.map(w => `<div class="fl-cal-weekday">${w}</div>`).join('')}</div><div class="fl-cal-days"></div>`;
    const daysGrid = wrap.querySelector('.fl-cal-days');
    for (let i = 0; i < startWeekDay; i++) {
        const empty = document.createElement('div');
        empty.className = 'fl-cal-day empty';
        daysGrid.appendChild(empty);
    }
    for (let d = 1; d <= lastDay.getDate(); d++) {
        const ds = `${year}-${String(month + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
        const hasEmoji = !!emojis[ds];
        const cell = document.createElement('div');
        cell.className = 'fl-cal-day' + (ds === todayStr ? ' today' : '') + (hasEmoji ? ' has-checkin' : '');
        cell.innerHTML = `<div class="fl-cal-date">${d}</div><div class="fl-cal-emoji">${emojis[ds] || ''}</div>`;
        cell.addEventListener('click', () => {
            state._flSelectedDate = ds;
            picker.querySelectorAll('.fl-emoji-option').forEach(el => el.classList.toggle('selected', el.textContent === emojis[ds]));
            toast('点击下方表情添加到该日');
        });
    ...
    picker.innerHTML = emojiList.map(e => `<span class="fl-emoji-option ${state._flSelectedDate && emojis[state._flSelectedDate] === e ? 'selected' : ''}" data-action="fl-emoji-pick" data-emoji="${e}">${e}</span>`).join('');
}

function flRenderRecipeList() {
    const container = document.getElementById('flRecipeList');
    if (!container) return;
    const fl = flGetData();
    const filter = state._flRecipeFilter || 'all';
    let recipes = fl.recipes || [];
    if (filter !== 'all') recipes = recipes.filter(r => r.category === filter);
    container.innerHTML = recipes.length
        ? recipes.map(r => `<div class="fl-recipe-item"><div class="fl-recipe-info"><div class="fl-recipe-name">${escapeHtml(r.name)}</div><div class="fl-recipe-meta">${escapeHtml(r.category)} · ${escapeHtml((r.ingredients || []).join('、'))}</div></div><div class="fl-recipe-actions"><button data-action="fl-recipe-use" data-id="${r.id}">➕ 使用</button><button data-action="fl-recipe-edit" data-id="${r.id}">✎</button><button data-action="fl-recipe-delete" data-id="${r.id}">✕</button></div></div>`).join('')
        : '<p class="fl-empty">暂无菜谱，点击上方新建</p>';
}

function flRenderCircHistory() {
    const wrap = document.getElementById('flCircHistoryWrap');
    if (!wrap) return;
    const fl = flGetData();
    const list = fl.circumferenceHistory || [];
    if (!list.length) { wrap.innerHTML = ''; return; }
    const fields = [
        { key: 'arm', label: '上臂' }, { key: 'waist', label: '腰围' }, { key: 'abdomen', label: '腹围' },
        { key: 'hip', label: '臀围' }, { key: 'thigh', label: '大腿' }, { key: 'calf', label: '小腿' }
    ];
    let html = '<table class="fl-circ-table"><thead><tr><th>日期</th>' + fields.map(f => `<th>${f.label}</th>`).join('') + '</tr></thead><tbody>';
    [...list].sort((a, b) => (a.date > b.date ? -1 : 1)).slice(0, 10).forEach(row => {
        html += `<tr><td>${flFormatTime(row.date)}</td>` + fields.map(f => `<td>${row[f.key] != null ? row[f.key] : '-'}</td>`).join('') + '</tr>';
    });
    html += '</tbody></table>';
    wrap.innerHTML = html;
}

function flRenderCharts() {
    const fl = flGetData();
    if (typeof Chart === 'undefined') return;
    // 稳定排序：先按打卡时间，同一天/同一秒完成多个阶梯时按阶梯目标方向二次排序，
    // 避免目标线因排序不稳定出现波动甚至「上涨」的错觉。
    const steps = flGenerateSteps(fl.user.startWeight, fl.user.targetWeight, fl.user.stepSize);
    const direction = fl.user.startWeight > fl.user.targetWeight ? -1 : 1;
    const weightRecords = [...(fl.stepRecords || [])].sort((a, b) => {
        const t = String(a.completedAt).localeCompare(String(b.completedAt));
        if (t !== 0) return t;
        return (a.target - b.target) * direction;
    });
    const weightLabels = weightRecords.map(r => r.completedAt.slice(5, 10).replace('-', '/'));
    const weightData = weightRecords.map(r => r.weight);
    const weightTarget = fl.user.targetWeight;
    if (document.getElementById('flWeightChart')) {
        if (state._flWeightChart) state._flWeightChart.destroy();
        state._flWeightChart = new Chart(document.getElementById('flWeightChart'), {
            type: 'line',
            data: {
                labels: weightLabels,
                datasets: [
                    { label: '体重 (斤)', data: weightData, borderColor: '#2D8C6F', backgroundColor: 'rgba(45,140,111,0.1)', tension: 0.3, fill: true },
                    { label: '目标 (斤)', data: weightRecords.map(r => r.target), borderColor: '#F59E0B', borderDash: [4, 4], pointRadius: 0 }
                ]
            },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } }, scales: { y: { beginAtZero: false } } }
        });
    }
    const circRecords = [...(fl.circumferenceHistory || [])].sort((a, b) => (a.date > b.date ? 1 : -1));
    const circLabels = circRecords.map(r => r.date.slice(5, 10).replace('-', '/'));
    if (document.getElementById('flCircChart')) {
        if (state._flCircChart) state._flCircChart.destroy();
        state._flCircChart = new Chart(document.getElementById('flCircChart'), {
            type: 'line',
            data: {
                labels: circLabels,
                datasets: [
                    { label: '腰围', data: circRecords.map(r => r.waist), borderColor: '#3B82F6', tension: 0.3 },
                    { label: '臀围', data: circRecords.map(r => r.hip), borderColor: '#EC4899', tension: 0.3 },
                    { label: '大腿', data: circRecords.map(r => r.thigh), borderColor: '#8B5CF6', tension: 0.3 }
                ]
            },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } }, scales: { y: { beginAtZero: false } } }
        });
...
function flHandleCheckin() {
    const fl = flGetData();
    const u = fl.user;
    const steps = flGenerateSteps(u.startWeight, u.targetWeight, u.stepSize);
    if (!steps.length) { toast('请先在「设置」中设置起始和目标体重'); return; }
    const input = document.getElementById('flTodayWeight');
    const inputWeight = parseFloat(input ? input.value : '');
    if (isNaN(inputWeight) || inputWeight <= 0) { toast('请输入有效的体重 (斤)'); return; }
    const direction = u.startWeight > u.targetWeight ? -1 : 1;
    const recMap = {};
    (fl.stepRecords || []).forEach(r => { recMap[Math.round((r.target || 0) * 100)] = r; });
    // 本次体重一次性覆盖所有「已达成」的阶梯（上行/下行方向自适应），无需逐阶多次点击
    const achieved = [];
    for (const t of steps) {
        if (recMap[Math.round(t * 100)]) continue; // 已打卡的阶梯跳过
        const reached = direction < 0 ? inputWeight <= t + 1e-9 : inputWeight >= t - 1e-9;
        if (reached) achieved.push(t);
    }
    if (achieved.length) {
        const frontier = direction < 0 ? Math.min.apply(null, achieved) : Math.max.apply(null, achieved);
        achieved.forEach(t => {
            fl.stepRecords.push({ target: t, weight: (t === frontier ? inputWeight : t), completedAt: flNowStr() });
        });
        flUpdateAchievements();
        flSaveData();
        render();
        toast(`✅ 已同步到 ${frontier.toFixed(1)} 斤（含上方 ${achieved.length} 个阶梯）`);
        const allDone = flGetStepStatus(steps, fl.stepRecords).every(s => s.status === 'completed');
        if (allDone) setTimeout(() => toast('🎉🎉🎉 恭喜你达成所有阶梯目标！'), 500);
        return;
    }
    // 未跨越任何新阶梯：保护当前阶梯不被误标
    const current = flGetCurrentStep(steps, fl.stepRecords);
    if (!current || current.status === 'done') { toast('🎉 所有阶梯已完成！请设置新目标'); return; }
    if (inputWeight > current.target + 1e-9) {
        toast(`当前体重 ${inputWeight.toFixed(1)} 斤，比当前目标 ${current.target.toFixed(1)} 斤高，本次未达成新阶梯，继续努力！`);
        return;
    }
    fl.stepRecords.push({ target: current.target, weight: inputWeight, completedAt: flNowStr() });
    flUpdateAchievements();
    flSaveData();
    render();
    toast(`✅ 打卡成功！完成阶梯 ${current.target.toFixed(1)} 斤`);
}

function flHandleDietToggle(idx) {
    const fl = flGetData();
    const meal = fl.diet.meals[idx];
    if (!meal) return;
    meal.status = meal.status === 'done' ? 'plan' : 'done';
    if (meal.status === 'done') {
        const today = flTodayStr();
        const emojis = ['🏋️', '🚴', '🏃', '🧘', '🏊', '🚶', '💪', '🤸', '⛹️', '🧗'];
        if (!fl.diet.checkinEmojis[today]) fl.diet.checkinEmojis[today] = emojis[Math.floor(Math.random() * emojis.length)];
    }
    flSaveData();
    render();
    toast(meal.status === 'done' ? '✅ 饮食打卡成功！' : '已取消打卡');
}

function flHandleDietSave() {
    const fl = flGetData();
    const idx = parseInt(document.getElementById('flDietEditIndex').value);
    const time = document.getElementById('flDietTime').value;
    const name = document.getElementById('flDietName').value.trim();
    const itemsStr = document.getElementById('flDietItems').value.trim();
    const status = document.getElementById('flDietStatus').value;
    const saveRecipe = document.getElementById('flDietSaveRecipe') && document.getElementById('flDietSaveRecipe').checked;
    if (!name) { toast('请输入餐名'); return; }
    const items = itemsStr ? itemsStr.split(/[,，]/).map(s => s.trim()).filter(Boolean) : [];
    const category = flMealCategory({ name });
    const meal = { time, name, items, status, category };
    if (idx >= 0 && idx < fl.diet.meals.length) {
        meal.note = fl.diet.meals[idx].note || '';
        fl.diet.meals[idx] = meal;
        toast('已更新');
    } else {
        meal.note = '';
        fl.diet.meals.push(meal);
        toast('已添加');
        // 同步创建菜谱，方便下次一键复用
        if (saveRecipe) {
            const exists = fl.recipes.find(r => r.name === name && r.category === category);
            if (!exists) {
                fl.recipes.push({ id: fl._nextRecipeId++, name, category, ingredients: items.slice() });
                toast('已同步到菜谱');
            }
        }
    }
    flSaveData();
    state._flShowDietForm = false;
    render();
}

function flHandleRecipeSave() {
    const fl = flGetData();
    const id = parseInt(document.getElementById('flRecipeEditId').value);
    const name = document.getElementById('flRecipeName').value.trim();
    const category = document.getElementById('flRecipeCategory').value;
    const ingredientsStr = document.getElementById('flRecipeIngredients').value.trim();
    if (!name) { toast('请输入菜谱名'); return; }
    const ingredients = ingredientsStr ? ingredientsStr.split(/[,，]/).map(s => s.trim()).filter(Boolean) : [];
    if (id >= 0) {
        const r = fl.recipes.find(x => x.id === id);
        if (r) { r.name = name; r.category = category; r.ingredients = ingredients; toast('已更新'); }
    } else {
        fl.recipes.push({ id: fl._nextRecipeId++, name, category, ingredients });
        toast('已创建');
    }
    flSaveData();
    state._flShowRecipeForm = false;
    render();
}

function flHandleSaveProfile() {
    const fl = flGetData();
    const u = fl.user;
    u.name = document.getElementById('flProfileName').value.trim();
    u.gender = document.getElementById('flProfileGender').value;
    u.age = parseFloat(document.getElementById('flProfileAge').value) || null;
    u.height = parseFloat(document.getElementById('flProfileHeight').value) || null;
    u.startWeight = parseFloat(document.getElementById('flProfileStart').value);
    u.targetWeight = parseFloat(document.getElementById('flProfileTarget').value);
    u.stepSize = parseFloat(document.getElementById('flProfileStepSize').value) || 0.2;
    u.bodyFat = parseFloat(document.getElementById('flProfileBodyFat').value) || null;
    u.visceralFat = parseFloat(document.getElementById('flProfileVisceral').value) || null;
    u.calorieBudget = parseFloat(document.getElementById('flProfileCalorieBudget').value) || 1500;
    u.note = document.getElementById('flProfileNote').value.trim();
    ['arm', 'waist', 'abdomen', 'hip', 'thigh', 'calf'].forEach(k => { u[k] = u[k] || null; });
    flSaveData();
    render();
    toast('档案已保存');
}

function flHandleSaveCirc() {
    const fl = flGetData();
    const row = {
        date: flNowStr(),
        arm: parseFloat(document.getElementById('flCircArm').value) || null,
        waist: parseFloat(document.getElementById('flCircWaist').value) || null,
        abdomen: parseFloat(document.getElementById('flCircAbdomen').value) || null,
        hip: parseFloat(document.getElementById('flCircHip').value) || null,
        thigh: parseFloat(document.getElementById('flCircThigh').value) || null,
        calf: parseFloat(document.getElementById('flCircCalf').value) || null
    };
    fl.circumferenceHistory.push(row);
    const u = fl.user;
    u.arm = row.arm; u.waist = row.waist; u.abdomen = row.abdomen; u.hip = row.hip; u.thigh = row.thigh; u.calf = row.calf;
    flSaveData();
    render();
    toast('围度已记录');
}

function flHandleReset(type) {
    const fl = flGetData();
    if (type === 'checkin') {
        if (!confirm('确定清空所有打卡记录？')) return;
        fl.stepRecords = [];
        fl.achievements.forEach(a => a.unlocked = false);
    } else if (type === 'diet') {
        if (!confirm('确定清空所有饮食记录？')) return;
        fl.diet.meals = [];
        fl.diet.checkinEmojis = {};
    } else if (type === 'all') {
        if (!confirm('确定重置全部减脂数据？此操作不可恢复。')) return;
        state.data.fatloss = null;
    }
    flSaveData();
    render();
    toast('已重置');
...
function flHandleRecipeUse(id) {
    const fl = flGetData();
    const recipe = fl.recipes.find(r => r.id === id);
    if (!recipe) return;
    fl.diet.meals.push({ time: new Date().toTimeString().slice(0, 5), name: recipe.name, items: recipe.ingredients.slice(), status: 'plan', note: '📖 来自菜谱', category: recipe.category || '其他' });
    flSaveData();
    render();
    toast(`已添加 "${recipe.name}"`);
}

function flHandleRefreshMotto() {
    const mottos = [
        '💪 每完成一个小阶梯，都是向目标迈进的一大步！',
        '🌟 今天的汗水，是明天更美的自己。',
        '🔥 坚持不是一时的热血，而是日复一日的自律。',
        '🌈 减脂路上，你比昨天更强大。',
        '✨ 小目标成就大改变。'
    ...
    if (next === fl.user.motto) next = mottos[(mottos.indexOf(next) + 1) % mottos.length];
    fl.user.motto = next;
    flSaveData();
    const el = document.getElementById('flMottoText');
...
function initEvents() {
    ...
    document.addEventListener('click', async e => {
        ...
        // ========== 健康减脂（fl-*） ==========
        if (action === 'fl-checkin') {
            flHandleCheckin();
            return;
        }
        if (action === 'fl-toggle-section') {
            const section = el.dataset.section;
            const c = flGetCollapsed();
            c[section] = !c[section];
            flSaveData();
            renderFatlossPage(state._flTab || 'fl-overview');
            return;
        }
        if (action === 'fl-refresh-motto') {
            flHandleRefreshMotto();
            return;
        }
        if (action === 'fl-diet-add') {
            state._flShowDietForm = true;
            state._flDietEditIndex = -1;
            render();
            return;
        }
        if (action === 'fl-diet-edit') {
            state._flShowDietForm = true;
            state._flDietEditIndex = parseInt(el.dataset.index, 10);
            render();
            return;
        }
        if (action === 'fl-diet-delete') {
            const idx = parseInt(el.dataset.index, 10);
            if (!isNaN(idx) && confirm('删除这条饮食记录？')) {
                flGetData().diet.meals.splice(idx, 1);
                flSaveData();
                render();
                toast('已删除');
            }
        ...
        if (action === 'fl-diet-toggle') {
            const idx = parseInt(el.dataset.index, 10);
            if (!isNaN(idx)) flHandleDietToggle(idx);
            return;
        }
        if (action === 'fl-diet-form-close') {
            state._flShowDietForm = false;
            render();
            return;
        }
        if (action === 'fl-diet-form-save') {
            flHandleDietSave();
            return;
        }
        if (action === 'fl-recipe-filter') {
            state._flRecipeFilter = el.dataset.cat;
            render();
            return;
        }
        if (action === 'fl-recipe-add') {
            state._flShowRecipeForm = true;
            state._flRecipeEditId = -1;
            render();
            return;
        }
        if (action === 'fl-recipe-edit') {
            state._flShowRecipeForm = true;
            state._flRecipeEditId = parseInt(el.dataset.id, 10);
            render();
            return;
        }
        if (action === 'fl-recipe-delete') {
            const id = parseInt(el.dataset.id, 10);
            const fl = flGetData();
            if (!isNaN(id) && confirm('删除此菜谱？')) {
                fl.recipes = fl.recipes.filter(r => r.id !== id);
                flSaveData();
                render();
                toast('已删除');
            }
        ...
        if (action === 'fl-recipe-use') {
            const id = parseInt(el.dataset.id, 10);
            if (!isNaN(id)) flHandleRecipeUse(id);
            return;
        }
        if (action === 'fl-recipe-form-close') {
            state._flShowRecipeForm = false;
            render();
            return;
        }
        if (action === 'fl-recipe-form-save') {
            flHandleRecipeSave();
            return;
        }
        if (action === 'fl-recipe-online') {
            state._flShowOnlineRecipes = true;
            state._flOnlineRecipeCat = '早餐';
            render();
            return;
        }
        if (action === 'fl-online-recipes-close') {
            state._flShowOnlineRecipes = false;
            render();
            return;
        }
        if (action === 'fl-online-recipe-cat') {
            state._flOnlineRecipeCat = el.dataset.cat;
            render();
            return;
        }
        if (action === 'fl-online-add') {
            const cat = el.dataset.cat;
            const idx = parseInt(el.dataset.idx, 10);
            const recipe = (FL_ONLINE_RECIPES[cat] || [])[idx];
            if (recipe) {
                const fl = flGetData();
                fl.diet.meals.push({ time: new Date().toTimeString().slice(0, 5), name: recipe.name, items: recipe.items.slice(), status: 'plan', category: cat, note: '🌐 来自联网推荐' });
                flSaveData();
                state._flShowOnlineRecipes = false;
                state._flDietSubTab = 'today';
                render();
                toast(`已添加 “${recipe.name}” 到今日饮食`);
            }
            return;
        }
        if (action === 'fl-online-save') {
            const cat = el.dataset.cat;
            const idx = parseInt(el.dataset.idx, 10);
            const recipe = (FL_ONLINE_RECIPES[cat] || [])[idx];
            if (recipe) {
                const fl = flGetData();
                const exists = fl.recipes.find(r => r.name === recipe.name && r.category === cat);
                if (!exists) {
                    fl.recipes.push({ id: fl._nextRecipeId++, name: recipe.name, category: cat, ingredients: recipe.items.slice() });
                    flSaveData();
                }
                toast(`已保存 “${recipe.name}” 到菜谱`);
            }
            return;
        }
        if (action === 'fl-diet-subtab') {
            state._flDietSubTab = el.dataset.subtab;
            render();
            return;
        }
        if (action === 'fl-cal-search') {
            flHandleCalSearch();
            return;
        }
        if (action === 'fl-cal-add') {
            if (el.dataset.name) flHandleCalAdd(el.dataset.name);
            return;
        }
        if (action === 'fl-emoji-pick') {
            const ds = state._flSelectedDate;
            if (!ds) { toast('请先点击日历中的某一天'); return; }
            flGetData().diet.checkinEmojis[ds] = el.dataset.emoji;
            flSaveData();
            render();
            toast(`已为 ${ds} 添加 ${el.dataset.emoji}`);
            return;
        }
        if (action === 'fl-save-profile') {
            flHandleSaveProfile();
            return;
        }
        if (action === 'fl-save-circ') {
            flHandleSaveCirc();
            return;
        }
        if (action === 'fl-save-motto') {
            const fl = flGetData();
            fl.user.motto = document.getElementById('flSettingsMotto').value.trim();
            flSaveData();
            toast('激励语已保存');
            return;
        }
        if (action === 'fl-reset-checkin') {
            flHandleReset('checkin');
            return;
        }
        if (action === 'fl-reset-diet') {
            flHandleReset('diet');
            return;
        }
        if (action === 'fl-reset-all') {
            flHandleReset('all');
            return;
        }
