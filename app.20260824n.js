...
function engSetCopyCount(v) {
    localStorage.setItem('engPlay_copyCount', String(v));
}
...
function engAddHistory(kind, item, maxLen) {
    const arr = engGetHistory(kind);
    arr.unshift(item);
    const m = maxLen || 50;
    const trimmed = arr.slice(0, m);
    localStorage.setItem('engPlay_' + kind, JSON.stringify(trimmed));
}
function engDeleteHistoryItem(kind, index) {
    ...
    arr.splice(index, 1);
    localStorage.setItem('engPlay_' + kind, JSON.stringify(arr));
...
function engSaveTableToHistory() {
    ...
    const arr = engGetHistory('copy');
    // 尝试更新最近一条同一句子的记录；否则新增
    ...
    localStorage.setItem('engPlay_copy', JSON.stringify(arr.slice(0, 50)));
...
function engRenderHistoryListHTML() {
    ...
    if (stateTab === 'search') {
        ...
        const renderCopyDetail = (item) => {
            ...
            html += `<div class="eng-copy-detail-line"><strong>英文翻译：</strong>${escapeHtml(item.en || '')}</div>`;
            if (item.pattern) {
            ...
            if (item.table && Array.isArray(item.table.rows) && item.table.rows.length) {
                ...
                html += `<div class="eng-empty">未保存详细表格</div>`;
            }
            html += `</div>`;
            return html;
        };
        ...
        if (action === 'eng-clear-history') {
            if (typeof uiConfirm === 'function') {
                uiConfirm('确定要清空英语游乐场所有历史记录吗？', { isDanger: true }).then(ok => {
                    if (ok) {
                        localStorage.removeItem('engPlay_search');
                        localStorage.removeItem('engPlay_copy');
                        render();
                    }
                });
            } else {
                if (confirm('确定要清空英语游乐场所有历史记录吗？')) {
                    localStorage.removeItem('engPlay_search');
                    localStorage.removeItem('engPlay_copy');
                    render();
                }
