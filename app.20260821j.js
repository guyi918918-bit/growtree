...
async function engTranslateAPI(zh) {
    ...
    if (r2) return r2;
    // 第3层：LibreTranslate 公共实例（免费、无需 key）
    ...
    if (mock) return mock;
    // 第5层：明确失败提示（不产生乱码）
    return '（翻译暂不可用：网络受限或未配置翻译API，可到「更多→翻译API设置」填百度密钥）';
}
...
function engRenderMore() {
    ...
    <div class="eng-card">
        ...
        <div class="eng-card-desc">填百度翻译开放平台 AppID 与密钥，翻译更准（浏览器环境会自动走 CORS 代理）</div>
        <div class="eng-row">
        ...
        <div class="eng-text-muted">🔗 在 https://fanyi-api.baidu.com 开放平台创建应用获取</div>
    </div>
    ...
    document.addEventListener('click', async e => {
        ...
        if (action === 'eng-baidu-test') {
            ...
            engTranslateAPI('你好').then(r => {
                if (st) st.textContent = r && r !== '你好' && !String(r).startsWith('[Translation]') ? `成功：${r}` : '未生效（未配置或网络受限）';
            });
            return;
        }
