...
function engRenderHistoryListHTML() {
    ...
    if (stateTab === 'search') {
        ...
        const renderCopyDetail = (item) => {
            let html = `<div style="margin-bottom:8px;"><strong>中文原句：</strong>${escapeHtml(item.zh || '')}</div>`;
            html += `<div style="margin-bottom:8px;"><strong>英文翻译：</strong>${escapeHtml(item.en || '')}</div>`;
            if (item.pattern) {
                html += `<div style="margin-bottom:8px;"><strong>句型分析：</strong>${escapeHtml(item.pattern)}</div>`;
            }
