...
function repairZeroBasePoints() {
    ...
    pts.history.forEach(h => {
        ...
        const recordTime = h.time || h.updatedAt || h.date;
        // 没有清零标记则全部修复；有清零标记只修复清零之后的记录
        if (clearedAt && recordTime && String(recordTime) <= String(clearedAt)) return;
...
function resolvePointsCalibOnSync(cloud, localClearedAt) {
    ...
    // 保留两端并集的真实打卡 base 记录（打卡完成状态不丢），仅把每条积分置 0。
    // 清零后新产生的真实打卡由启动时的 repairZeroBasePoints 自动恢复分值。
    const baseRecords = (state.data.points.history || []).filter(h => h.type === 'base');
    state.data.points.history = baseRecords.map(h => ({ ...h, points: 0 }));
