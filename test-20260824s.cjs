...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824s.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824s.css'), 'utf8');

const BUILD = '20260824s';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 85) ok('SW CACHE 已递增到 v85'); else no('SW CACHE 未递增');
...
if (HTML.includes('app.20260824s.js') && HTML.includes('chart.js')) ok('index.html 加载 chart.js 与 s 版本 app'); else no('index.html 引用不正确');
try {
    run(`
        state.data.fatloss = {
            user: { name: '测试用户', gender: '女', age: 28, height: 163, startWeight: 120, targetWeight: 100, stepSize: 0.2, bodyFat: null, visceralFat: null, arm: null, waist: null, abdomen: null, hip: null, thigh: null, calf: null, note: '', motto: '加油！' },
            stepRecords: [], circumferenceHistory: [],
            diet: { meals: [], checkinEmojis: {} },
