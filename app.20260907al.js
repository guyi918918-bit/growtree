...
const BEAUTY_COMMON_CITIES = ['成都', '北京', '上海', '广州', '深圳', '杭州', '南京', '武汉', '西安', '重庆', '苏州', '天津', '长沙', '青岛', '厦门', '昆明', '哈尔滨', '沈阳', '大连', '郑州', '济南', '合肥', '福州', '南昌', '贵阳', '南宁', '兰州', '银川', '西宁', '乌鲁木齐', '拉萨', '呼和浩特', '石家庄', '太原', '长春', '海口', '宁波', '无锡', '常州', '温州', '泉州', '佛山', '东莞', '烟台', '唐山', '徐州', '南通', '潍坊', '淄博', '绍兴', '嘉兴', '台州', '金华', '惠州', '中山', '珠海', '汕头', '湛江', '三亚', '柳州', '桂林', '台北（中国台湾）', '香港', '中国澳门'];
...
function renderBeautyToday() {
    ...
    const selectedOther = city && !BEAUTY_COMMON_CITIES.includes(city);
    const citySelectOptions = BEAUTY_COMMON_CITIES.map(c => `<option value="${escapeHtml(c)}" ${c === city ? 'selected' : ''}>${escapeHtml(c)}</option>`).join('');
    ...
    return `
        <div class="card-header" style="align-items:flex-start;gap:8px">
            ...
            <div class="beauty-city-bar" style="display:flex;align-items:center;gap:6px;flex-shrink:0">
                <select id="beautyCitySelect" class="input" data-action="beauty-city-change" style="min-width:88px;width:auto;padding:5px 8px;font-size:12px;border-radius:8px;background:var(--card-bg);border:1px solid var(--border-color)">
                    ${city ? `<option value="" ${!city ? 'selected' : ''}>选择城市</option>` : ''}
                    ${citySelectOptions}
                    <option value="__other__" ${selectedOther ? 'selected' : ''}>其他城市</option>
                </select>
                <input type="text" id="beautyCityInput" class="input" placeholder="输入城市名" value="${escapeHtml(selectedOther ? city : '')}" style="display:${selectedOther ? 'inline-block' : 'none'};width:80px;padding:5px 8px;font-size:12px;border-radius:8px;background:var(--card-bg);border:1px solid var(--border-color)">
                <button class="btn btn-secondary" data-action="beauty-city-confirm" style="font-size:12px;padding:5px 10px;white-space:nowrap">确定</button>
