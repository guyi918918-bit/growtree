...
async function engFetchBaidu(appid, key, zh) {
    ...
    for (const wrap of proxies) {
        try {
            const res = await fetch(wrap(base), { mode: 'cors' });
            if (!res.ok) continue;
            const data = await res.json();
...
function initEvents() {
    ...
    document.addEventListener('click', async e => {
        ...
        if (action === 'eng-baidu-test') {
            ...
            engTranslateAPI('你好').then(r => {
                if (!st) return;
                if (r && r !== '你好' && !String(r).startsWith('[Translation]') && !String(r).startsWith('（')) {
                    st.textContent = `✅ 成功：${r}`;
                } else if (String(r).startsWith('（')) {
                    st.textContent = '未生效：网络受限，请检查网络';
                } else {
                    st.textContent = '未生效：密钥可能不正确';
                }
            });
            return;
        }
