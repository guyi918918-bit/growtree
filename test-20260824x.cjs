...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824x.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824x.css'), 'utf8');

const BUILD = '20260824x';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 90) ok('SW CACHE 已递增到 v90'); else no('SW CACHE 未递增');
...
try {
    ...
    if (clearedEarned === 0 && clearedUsed === 0 && clearedHistoryLen === 0 &&
        ...
        ok('clearAllPointsAndRankData 彻底清零积分/段位/赛季/成就/流水/心愿');
    } else {
        no('clearAllPointsAndRankData 清零不完整：' + JSON.stringify({
            clearedEarned, clearedUsed, clearedHistoryLen, clearedRankStars, clearedAccounted,
            clearedCourage, clearedAchv, clearedTitles, clearedWinStreak, clearedSeasonPeak,
            clearedSeasonHistory, wishExchanged, calibrated, clearedAt
        }));
    }
} catch (e) {
...
// 回归测试：清零后再同步云端旧 4w 数据不会回潮
// 模拟 syncFromCloudCore 合并云端污染数据，验证本地清零标记能压制旧数据
try {
    run(`
        // 先清零
        clearAllPointsAndRankData();
        // 备份清零后的本地状态
        var localCleared = JSON.parse(JSON.stringify(state.data));
        // 模拟云端 payload（旧 4w 污染数据）
        var cloudPayload = {
            points: { earned: 40000, used: 0, history: [] },
            game: { rankStars: 421, accountedEarnedStars: 421, _lastSyncEarned: 40000, _pointsCalibrated: '20260824v' },
            wishes: [], makeupLog: {}, updatedAt: '2026-01-01T00:00:00.000Z'
        };
        for (var i = 0; i < 400; i++) cloudPayload.points.history.push({ id: 'c' + i, type: 'base', points: 100, date: '2026-01-01', checkInId: 'x' + i });
        // 直接调用合并后的冲突解决逻辑：本地校准 build 更新，应当保留本地清零状态
        var localCalib = state.data.game._pointsCalibrated;
        var cloudCalib = cloudPayload.game._pointsCalibrated;
        function calibBuildCompare(a, b) {
            if (!a) return b ? -1 : 0;
            if (!b) return 1;
            return String(a).localeCompare(String(b));
        }
        if (calibBuildCompare(localCalib, cloudCalib) >= 0) {
            // 本地清零状态优先，不应被云端旧数据覆盖
            state.data.points = localCleared.points;
            state.data.game = { ...state.data.game, ...localCleared.game };
            state.data.wishes = localCleared.wishes;
            state.data.makeupLog = localCleared.makeupLog;
        } else {
            // 云端更新，采纳云端（测试中不应走到这里）
            state.data.points = cloudPayload.points;
            state.data.game = { ...state.data.game, ...cloudPayload.game };
            state.data.wishes = cloudPayload.wishes;
            state.data.makeupLog = cloudPayload.makeupLog;
        }
        recalcPoints();
        recalcCourage();
    `);
    ...
    const afterHistoryLen = run('state.data.points.history.length');
    if (afterEarned === 0 && afterRankStars === 0 && afterHistoryLen === 0) {
        ok('清零后云端旧 4w 数据不会回潮（本地清零标记压制旧数据）');
    } else {
        no('清零后云端数据回潮：' + JSON.stringify({ afterEarned, afterRankStars, afterHistoryLen }));
    }
} catch (e) {
