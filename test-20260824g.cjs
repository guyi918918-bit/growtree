...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824g.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');

const BUILD = '20260824g';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 73) ok('SW CACHE 已递增到 v73'); else no('SW CACHE 未递增');
...
const dom = new JSDOM('<!DOCTYPE html><html><head></head><body><div id="app"><main class="main"><div id="content"></div></main></div><div id="settingsModal" style="display:none"></div><div id="uiModal" style="display:none"><div class="ui-modal-card"><div class="ui-modal-title"></div><div class="ui-modal-msg"></div><input id="uiModalInput" style="display:none"><div class="ui-modal-actions"><button id="uiModalCancel">取消</button><button id="uiModalOk">确定</button></div></div></div><div id="toast"></div></body></html>', {
