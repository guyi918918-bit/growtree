...
function ensureGameDefaults() {
    ...
    // 关键修复：旧数据迁移时，accountedEarnedStars 必须同时覆盖当前 earned 积分和当前段位，
    // 否则 recalcPoints 之后 earned 被重算为很大的值，会一次性把青铜用户送到王者。
    if (typeof g.accountedEarnedStars !== 'number') {
        const earnedStars = Math.floor((state.data.points.earned || 0) / 100);
        g.accountedEarnedStars = Math.max(g.rankStars || 0, earnedStars);
    }
    // 兜底：accountedEarnedStars 不能低于 rankStars，否则 syncEarnedStars 会重复加星。
    if (g.accountedEarnedStars < g.rankStars) {
        g.accountedEarnedStars = g.rankStars;
    }
...
function syncEarnedStars() {
    ...
    // 单次启动最多把 10 颗星从 earned 增量兑换成段位星，防止脏数据/异常流水导致青铜直接飞升王者。
    const MAX_SYNC_STARS_PER_SESSION = 10;
    if (diff > 0) {
...
function initGame() {
    ensureGameDefaults();
    recalcPoints();
    // 再次校准：确保 accountedEarnedStars 不低于 earned/100，防止历史 earned 在 syncEarnedStars 中被追溯成星。
    const g = state.data.game;
    const earnedStars = Math.floor((state.data.points.earned || 0) / 100);
    if ((g.accountedEarnedStars || 0) < earnedStars) {
        g.accountedEarnedStars = earnedStars;
    }
    if ((g.accountedEarnedStars || 0) < (g.rankStars || 0)) {
    ...
    recalcCourage();
    updateSeasonPeak();
    // 异常段位校准：若本赛季起点存在，且当前星数比起点高出超过 100 颗星（10000 积分），
    // 视为脏数据/异常飞升，自动回滚到赛季起点并同步 accountedEarnedStars，避免王者幽灵段位。
    const seasonStart = g.season && typeof g.season.startStars === 'number' ? g.season.startStars : null;
    if (seasonStart !== null && (g.rankStars || 0) > seasonStart + 100) {
        g.rankStars = seasonStart;
        g.accountedEarnedStars = Math.max(seasonStart, g.accountedEarnedStars || 0);
        updateSeasonPeak();
        console.warn('[growtree] rankStars anomaly calibrated:', seasonStart);
    }
}
...
function engGenerateOutput(zh, rowCount) {
    // 改为 3 列 × N 行：列 = 英文 / 中文 / 备注，行 = 各个语法成分
    // 删除按钮不再以表格列呈现，而是每行右侧悬浮的铅笔图标，点击后显示 ✕。
    ...
    for (let r = 0; r < rowCount; r++) {
        ...
        html += `<td class="eng-edit-cell eng-edit-cell-last"><input type="text" class="eng-cell-input" data-row="${r}" data-col="2" placeholder="备注" value="">
            <span class="eng-row-actions">
                ...
                <button class="eng-row-delete" data-action="eng-delete-row" data-row="${r}" title="删除此行" aria-label="删除此行">✕</button>
            </span>
        </td>`;
        html += `</tr>`;
    }
...
function engRowEditToggle(rowIdx) {
    ...
    const trs = table.querySelectorAll('tbody tr');
    trs.forEach((tr, idx) => {
        const isTarget = idx === rowIdx;
        tr.classList.toggle('eng-row-editing', isTarget);
    });
}
...
function engDeleteRow(rowIdx) {
    ...
    output.innerHTML = engGenerateOutput(zh, newRowCount);
    const newTable = document.getElementById('engManualTable');
    const trs = newTable.querySelectorAll('tbody tr');
    trs.forEach((tr, r) => {
        const labelInp = tr.querySelector('td.eng-row-label input.eng-label-input');
        if (labelInp && newLabels[r] !== undefined) labelInp.value = newLabels[r];
        const inputs = tr.querySelectorAll('td.eng-edit-cell input');
        inputs.forEach((inp, c) => {
            if (newRows[r] && newRows[r][c] !== undefined) inp.value = newRows[r][c];
        });
    });
    if (old.en) {
        const enSentence = document.getElementById('engUserEnglishSentence');
            ...
            engRunLesson().then(() => {
                setTimeout(() => {
                    if (engCurrentLesson.zh) {
                        ...
                        engSetCopyCount(c);
                        engAddHistory('copy', { time: new Date().toISOString(), zh: engCurrentLesson.zh, en: engCurrentLesson.en, detail: engCurrentLesson.fullHtml });
                        const todayStats = engGetTodayStats();
                        todayStats.copies += 1;
                        engSaveTodayStats(todayStats);
                        const monthData = engGetMonthData();
    ...
    if (stateTab === 'search') {
        ...
        if (!filtered.length) {
            ...
            itemsHtml = filtered.map(item => {
                ...
                <div class="eng-history-detail" id="eng-copy-detail-${realIndex}">${item.detail || '无详细分析'}</div>`;
            }).join('');
        }
    }
    return itemsHtml;
