...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824t.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824t.css'), 'utf8');

const BUILD = '20260824t';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 86) ok('SW CACHE 已递增到 v86'); else no('SW CACHE 未递增');
...
if (HTML.includes('app.20260824t.js') && HTML.includes('chart.js')) ok('index.html 加载 chart.js 与 t 版本 app'); else no('index.html 引用不正确');
try {
    run(`
        state.data.fatloss = {
