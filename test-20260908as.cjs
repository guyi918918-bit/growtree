...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260908as.js'), 'utf8');
...
// 1. 版本号
if (VERSION.build === '20260908as') ok('version.json = as'); else no('version.json 不是 as: ' + VERSION.build);
if (APP_JS.includes("const APP_BUILD = '20260908as';")) ok('APP_BUILD = as'); else no('APP_BUILD 不是 as');
if (SW_JS.includes("CACHE = 'growtree-shell-v111'") && SW_JS.includes('app.20260908as.js') && SW_JS.includes('styles.20260908as.css')) ok('sw.js v111 引用 as'); else no('sw.js 引用异常');
...
// as 新增：UI 提醒升级
if (YJXS_HTML.includes('cal-ledger-row-over')) ok('CSS 含摄入超标整行变红'); else no('CSS 缺整行变红');
if (YJXS_HTML.includes('cal-alert-banner')) ok('CSS 含警示横幅'); else no('CSS 缺警示横幅');
if (YJXS_HTML.includes('cal-summary-bad')) ok('CSS 含汇总区变红'); else no('CSS 缺汇总区变红');
if (YJXS_HTML.includes('别再吃了')) ok('文案含"别再吃了"'); else no('文案缺劝阻语');
if (YJXS_HTML.includes('目前摄入OK')) ok('文案含"目前摄入OK"'); else no('文案缺鼓励语');
if (YJXS_HTML.includes("elRow.className='cal-ledger-row")) ok('refresh 会切换摄入行颜色'); else no('refresh 未切换摄入行颜色');
if (YJXS_HTML.includes('elSum.className')) ok('refresh 会切换汇总区颜色'); else no('refresh 未切换汇总区颜色');
