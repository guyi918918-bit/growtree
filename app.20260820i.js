            ...
            <div class="eg-card">
                ...
                <div class="eg-card-body">
                    <div class="eg-symbol-table">
                        ...
                        <div class="eg-symbol-row"><span class="eg-symbol">🌌</span><span class="eg-name">Galaxy</span><span class="eg-level">1024 级</span></div>
                    </div>
...
function initEnglishPlayground() {
            ...
            function getTitle(level) {
                if (level >= 999) return '🏆 Omniglot 万能通';
            ...
            async function translateWithAPI(zh) {
                const apis = [
                    `https://api.mymemory.translated.net/get?q=${encodeURIComponent(zh)}&langpair=zh|en&de=demo@example.com`,
                    `https://libretranslate.com/translate?q=${encodeURIComponent(zh)}&source=zh&target=en`
                ];
                for (const url of apis) {
                    try {
                        ...
                        const data = await res.json();
                        let trans = null;
                        if (data.responseData && data.responseData.translatedText) trans = data.responseData.translatedText;
                        else if (data.translatedText) trans = data.translatedText;
            ...
            function analyzeSentence(zh, en) {
                ...
                if (/了|过/.test(zh) && !/正在/.test(zh) && !/每天/.test(zh) && !/经常/.test(zh)) {
                    result.tense = '一般过去时（表示已完成的动作）';
                    result.mistake = '动词用过去式（-ed 或不规则），时间状语如"昨天"放句首或句末。';
                } else if (/正在/.test(zh) || /在.*ing/.test(zh)) {
                    result.tense = '现在进行时（表示正在进行的动作）';
                    result.mistake = '结构：am/is/are + 现在分词（doing），注意主谓一致。';
                } else if (/每天|经常|总是|通常/.test(zh)) {
                    result.tense = '一般现在时（表示习惯性/重复性动作）';
                    result.mistake = '主语为三单（he/she/it）时，动词加 -s/-es。';
                } else if (/明天|将会|打算/.test(zh)) {
                    result.tense = '一般将来时（表示将要发生的动作）';
                    result.mistake = '用 will + 动词原形，或 be going to + 动词原形。';
                } else {
                    result.tense = '一般现在时（表示状态或一般事实）';
                    result.mistake = '注意主谓一致，三单加 -s。';
                }

                const words = en.split(' ');

                if (/^i (am|was) /i.test(en) || /^he (is|was) /i.test(en) || /^she (is|was) /i.test(en) ||
                    /^it (is|was) /i.test(en) || /^you (are|were) /i.test(en) || /^we (are|were) /i.test(en) ||
                    /^they (are|were) /i.test(en)) {
                    result.sentenceType = '主系表 (主语 + 系动词 + 表语)';
                    const parts = en.match(/^(\S+)\s+(is|am|are|was|were)\s+(.+)$/);
                    if (parts) {
                        const subj = parts[1];
                        const verb = parts[2];
                        const rest = parts[3];
                        result.bareSentence =
                        `${subj} ${verb} ${rest.split(/ (at|in|on|with|for|to|after|before|during|since|until) /)[0]}`;
                        result.wordOrder = `主语 (${subj}) → 系动词 (${verb}) → 表语 (${rest})`;
                        result.coreGrammar = '系动词连接主语和表语，描述主语的状态、特征或身份。表语可以是名词、形容词或介词短语。';
                        result.components = [
                            { role: '主语 (Subject)', text: subj },
                            { role: '系动词 (Linking Verb)', text: verb },
                            { role: '表语 (Predicative)', text: rest }
                        ];
                        const advMatch = rest.match(/\s+(at|in|on|with|for|to|after|before|during|since|until)\s+(.+)$/);
                        if (advMatch) {
                            result.components.push({ role: '状语 (Adverbial)', text: advMatch[0].trim() });
                        }
                    }
                } else if (/\b(will|shall|would|should|can|could|may|might|must|have|has|had)\b/i.test(en) && !/\b(is|am|are|was|were)\b/i.test(en)) {
                    result.sentenceType = '主谓宾 (含情态动词/助动词)';
                    const match = en.match(/^(\S+)\s+(will|shall|would|should|can|could|may|might|must|have|has|had)\s+(\w+)\s+(.*)$/i);
                    if (match) {
                        const subj = match[1];
                        const aux = match[2];
                        const verb = match[3];
                        const rest = match[4] || '';
                        result.bareSentence =
                            `${subj} ${aux} ${verb} ${rest.split(/ (at|in|on|with|for|to|after|before|during|since|until) /)[0]}`;
                        result.wordOrder = `主语 (${subj}) → 助动词/情态动词 (${aux}) → 动词原形 (${verb}) → 宾语/补语 (${rest})`;
                        result.coreGrammar =
                            `${aux} 是情态动词/助动词，后面跟动词原形。情态动词表示可能性、必要性或意愿；助动词用于构成时态。`;
                        result.components = [
                            { role: '主语 (Subject)', text: subj },
                            { role: '助动词/情态动词 (Auxiliary/Modal)', text: aux },
                            { role: '谓语动词 (Main Verb)', text: verb },
                            { role: '宾语/补语 (Object/Complement)', text: rest || '无' }
                        ];
                        const advMatch = rest.match(/\s+(at|in|on|with|for|to|after|before|during|since|until)\s+(.+)$/);
                        if (advMatch) {
                            result.components.push({ role: '状语 (Adverbial)', text: advMatch[0].trim() });
                ...
                } else if (/\b(went|ate|drank|walked|played|watched|saw|visited|had|took|made|bought|ran|swam|sang|danced|jumped|laughed|drew|colored|studied|worked|talked|listened|wrote|read|got|gave|told|asked|helped|called|met|found|thought|knew|felt|became|came|left|spoke|put|set|began|brought|built|caught|chose|drove|fell|flew|forgot|froze|grew|hid|held|kept|led|lay|lost|paid|rode|rose|saw|shook|shut|sang|sank|sat|spent|stood|stole|swam|swung|took|taught|tore|threw|woke|won|wrung)\b/i
                    .test(en) && !/\b(will|shall|is|am|are|was|were)\b/i.test(en)) {
                    result.sentenceType = '主谓宾 (一般过去时)';
                    const parts = en.match(/^(\S+)\s+(\w+)\s+(.*)$/);
                    if (parts) {
                        const subj = parts[1];
                        const verb = parts[2];
                        const rest = parts[3] || '';
                        result.bareSentence =
                            `${subj} ${verb} ${rest.split(/ (at|in|on|with|for|to|after|before|during|since|until|yesterday|today|last|ago) /)[0]}`;
                        result.wordOrder = `主语 (${subj}) → 谓语 (${verb} 过去式) → 宾语/补语 (${rest})`;
                        result.coreGrammar = '一般过去时表示过去发生的动作，动词用过去式。规则动词加 -ed，不规则动词需单独记忆。';
                        result.components = [
                            { role: '主语 (Subject)', text: subj },
                            { role: '谓语动词 (Verb - Past Tense)', text: verb },
                            { role: '宾语/补语 (Object/Complement)', text: rest || '无' }
                        ];
                        const advMatch = rest.match(
                            /\s+(at|in|on|with|for|to|after|before|during|since|until|yesterday|today|last|ago)\s+(.+)$/
                            );
                        if (advMatch) {
                            result.components.push({ role: '状语 (Adverbial)', text: advMatch[0].trim() });
                        }
                    }
                } else if (/\b(is|am|are|was|were)\s+\w+ing\b/i.test(en)) {
                    result.sentenceType = '主谓宾 (进行时)';
                    const parts = en.match(/^(\S+)\s+(is|am|are|was|were)\s+(\w+ing)\s+(.*)$/);
                    if (parts) {
                        const subj = parts[1];
                        const aux = parts[2];
                        const verb = parts[3];
                        ...
                        result.bareSentence =
                            `${subj} ${aux} ${verb} ${rest.split(/ (at|in|on|with|for|to|after|before|during|since|until) /)[0]}`;
                        result.wordOrder = `主语 (${subj}) → be动词 (${aux}) → 现在分词 (${verb}) → 宾语/状语 (${rest})`;
                        result.coreGrammar = '进行时表示正在进行的动作，结构：be + doing。现在分词表示动作正在进行。';
                        result.components = [
                            { role: '主语 (Subject)', text: subj },
                            { role: '助动词 (Auxiliary - be)', text: aux },
                            { role: '现在分词 (Present Participle)', text: verb },
                            { role: '宾语/状语 (Object/Adverbial)', text: rest || '无' }
                        ];
                        const advMatch = rest.match(
                            /\s+(at|in|on|with|for|to|after|before|during|since|until|now|today)\s+(.+)$/);
                        if (advMatch) {
                            result.components.push({ role: '状语 (Adverbial)', text: advMatch[0].trim() });
                        }
                    }
                } else if (/\b(and|with)\b/i.test(en) && !/\b(is|am|are|was|were)\b/i.test(en) && !/\b(ing)\b/i.test(en)) {
                    result.sentenceType = '主谓 (+ 伴随状语/并列结构)';
                    const subj = words[0] || '';
                    const rest = words.slice(1).join(' ');
                    result.bareSentence = `${subj} ${rest.split(/ (with|and) /)[0]}`;
                    result.wordOrder =
                        `主语 (${subj}) → 谓语 (${rest.split(/ (with|and) /)[0]}) → 伴随状语/并列 (${rest.replace(subj, '').replace(rest.split(/ (with|and) /)[0], '').trim()})`;
                    result.coreGrammar = '一般现在时，动词用原形（主语非三单）。并列结构用 and 连接两个动作，伴随状语用 with 表示"和...一起"。';
                    result.components = [
                        { role: '主语 (Subject)', text: subj },
                        { role: '谓语 (Predicate)', text: rest.split(/ (with|and) /)[0] || rest },
                    ];
                    const restPart = rest.replace(rest.split(/ (with|and) /)[0], '').trim();
                    if (restPart) {
                        result.components.push({ role: '伴随状语/并列 (Adverbial/Coordinate)', text: restPart });
                    }
                } else {
                    result.sentenceType = '主谓宾 (一般现在时)';
                    const subj = words[0] || '';
                    const verb = words[1] || '';
                    const rest = words.slice(2).join(' ') || '';
                    result.bareSentence =
                        `${subj} ${verb} ${rest.split(/ (at|in|on|with|for|to|after|before|during|since|until) /)[0]}`;
                    result.wordOrder = `主语 (${subj}) → 谓语 (${verb}) → 宾语/状语 (${rest})`;
                    result.coreGrammar = '一般现在时，主语三单（he/she/it）时动词加 -s/-es，其他情况用原形。';
                    result.components = [
                        { role: '主语 (Subject)', text: subj },
                        { role: '谓语动词 (Verb)', text: verb },
                        { role: '宾语/状语 (Object/Adverbial)', text: rest || '无' }
                    ];
                    const advMatch = rest.match(
                        /\s+(at|in|on|with|for|to|after|before|during|since|until|every|today|now)\s+(.+)$/);
                    if (advMatch) {
                        result.components.push({ role: '状语 (Adverbial)', text: advMatch[0].trim() });
                    }
                }
            ...
            function generateExamples(en) {
                if (/\bwent\b/.test(en) || /\bwatched\b/.test(en) || /\bhad\b/.test(en)) {
                    return [
                ...
                } else if (/\b(with|and)\b/.test(en) && !/\b(going|eating|drinking|running)\b/.test(en)) {
                    return [
                ...
                } else if (/\b(is|am|are|was|were)\s+\w+ing\b/.test(en)) {
                    return [
                ...
                } else if (/\b(will|shall|can|could|may|might|must|have|has|had)\b/i.test(en)) {
                    return [
            ...
            function generateFullLesson(zh, en) {
                const analysis = analyzeSentence(zh, en);
                let html = '';

                html +=
                    `<div class="eg-block"><span class="eg-label">📌 中文：</span>${zh}<br><span class="eg-label">🌐 英文：</span><span class="eg-en">${en}</span></div>`;

                html +=
                    `<div class="eg-block"><span class="eg-label">⏳ 时态选择：</span>${analysis.tense}<br><span class="eg-label">⚠️ 易错提醒：</span>${analysis.mistake}</div>`;

                html += `<div class="eg-block"><span class="eg-label">📐 句型判定：</span>${analysis.sentenceType}<br>`;
                html += `<span class="eg-label">🌱 裸句（主干）：</span>${analysis.bareSentence}<br>`;
                html += `<span class="eg-label">🔁 英文语序：</span>${analysis.wordOrder}<br>`;
                html += `<span class="eg-label">📖 核心知识点：</span>${analysis.coreGrammar}<br>`;

                html += `<span class="eg-label">🧩 成分拆解：</span><br>`;
                if (analysis.components.length > 0) {
                    analysis.components.forEach(comp => {
                        html +=
                        `<div class="eg-component-line">• <strong>${comp.role}</strong>：${comp.text}</div>`;
                    });
                } else {
                    html += `<div class="eg-component-line">• 暂无详细成分信息</div>`;
                }
                html += `</div>`;
                ...
                examples.forEach((ex, i) => {
                    html +=
                        `<div class="eg-example"><span class="eg-en">${i+1}. ${ex.en}</span><br><span class="eg-zh">${ex.zh}</span></div>`;
                });
                html += `</div>`;
            ...
            async function runLesson() {
                ...
                lessonOutput.innerHTML = `<div class="eg-empty">⏳ 正在生成逐句精讲...</div>`;
                const en = await translateWithAPI(zh);
                const html = generateFullLesson(zh, en);
