...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824u.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824u.css'), 'utf8');

const BUILD = '20260824u';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 87) ok('SW CACHE 已递增到 v87'); else no('SW CACHE 未递增');
...
if (HTML.includes('app.20260824u.js') && HTML.includes('chart.js')) ok('index.html 加载 chart.js 与 u 版本 app'); else no('index.html 引用不正确');
try {
    run(`
        state.data.fatloss = {
...
try {
    run(`
        ...
        state._flDietSubTab = 'recipes';
        render();
    `);
    const content = document.getElementById('content').innerHTML;
    if (content.includes('今日饮食') && content.includes('fl-diet-subtab') && content.includes('菜谱')) ok('饮食页可渲染'); else no('饮食页渲染异常');
    if (content.includes('flCalInput') && content.includes('flCalEngine')) ok('热量搜索栏已渲染'); else no('热量搜索栏缺失');
    if (content.includes('测试菜谱')) ok('菜谱列表已渲染'); else no('菜谱列表缺失');
} catch (e) {
