...
function initEnglishPlayground() {
            ...
            async function translateWithAPI(zh) {
                // 1. Google 翻译（gtx 非官方接口，质量稳定，优先使用）
                try {
                ...
                // 3. 本地词典兜底
                return mockTranslate(zh);
            }

            async function checkGrammarWithAPI(en) {
                try {
                    const res = await fetch('https://api.languagetool.org/v2/check', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json' },
                        body: `language=en-US&text=${encodeURIComponent(en)}`
                    });
                    const data = await res.json();
                    return (data.matches || []).filter(m => m.rule && m.rule.issueType !== 'style').slice(0, 5);
                } catch (e) {
                    return [];
                }
            }
            ...
            function sanityCheck(zh, en) {
                ...
                if (/斤/.test(zh) && /\b(pound|pounds|lb|lbs)\b/.test(lowerEn)) {
                    warnings.push('中文出现「斤」，译文却用了 pound(s)，可能存在单位换算或文化差异。和外国人说体重/重量时，建议直接用 jin 或补充为 kg。');
                }
                if (/减肥到/.test(zh) && /\blose\b/.test(lowerEn) && /\b(pound|pounds|kg|kilogram)\b/.test(lowerEn)) {
                    warnings.push('「减肥到 X 斤」强调的是目标体重，而不是「减掉 X 斤/磅」。更地道的表达可考虑 weigh X jin / get down to X jin。');
                }
                if (/公里|千米/.test(zh) && /\bmile|\bmiles\b/.test(lowerEn)) {
                    warnings.push('中文出现「公里/千米」，译文却用了 mile(s)，单位可能不匹配。');
                }
                // 虚拟语气提示
                if (/我希望/.test(zh) && /\bi\s+wish\b/.test(lowerEn)) {
                    warnings.push('「I wish (that)...」后面常用虚拟语气（与现在事实相反用过去式），表示难以实现或不太现实的愿望，不是一般过去时。');
                }
                // 进行时提示
                if (/正在/.test(zh) && !/\b(am|is|are|was|were)\s+\w+ing\b/.test(lowerEn)) {
                    warnings.push('中文有「正在」，英文却没使用进行时（be + doing），建议检查是否遗漏。');
                }
                // 逗号粘连（简单启发：两个独立分句用逗号连接且无连词）
                if (/，/.test(zh) && /,/.test(en) && !/[;:.?!]/.test(en.replace(/,.*,/, ''))) {
                    ...
                    if (clauses.length >= 2 && !/\b(and|but|or|so|yet|for|nor|because|since|as|if|when|while|although|though)\b/i.test(en)) {
                        warnings.push('英文两个独立分句只用逗号连接（comma splice）是常见语法错误，建议用句号、分号或加上 and/but/so 等连词。');
                    }
                }
                return warnings;
            }
            ...
            function analyzeSentence(zh, en) {
                ...
                if (/\b(if|wish|as if|suppose)\b/.test(lower) && /\b(would|could|were|had)\b/.test(lower)) {
                    result.tense = '虚拟语气';
                    result.coreGrammar = '虚拟语气用于表达与事实相反、难以实现或主观的愿望/假设。wish 后的宾语从句常用过去式表示现在的愿望。';
                    ...
                    result.tense = '过去进行时';
                    result.coreGrammar = '过去进行时表示过去某时正在进行的动作，结构为 was/were + doing。';
                } else if (/\b(have|has)\s+\w+ed\b/.test(lower) || /\b(have|has)\s+(been|gone|done|seen|made|taken|eaten|written|known|come|become|fallen|given|lain|risen|shaken|shrunk|slain|stolen|swum|thrown|woken|won)\b/.test(lower)) {
                    result.tense = '现在完成时';
                    result.coreGrammar = '现在完成时表示过去发生的动作对现在造成的影响，或从过去持续到现在，结构为 have/has + 过去分词。';
                } else if (/\bhad\s+\w+ed\b/.test(lower) || /\bhad\s+(been|gone|done|seen|made|taken|eaten|written)\b/.test(lower)) {
                    result.tense = '过去完成时';
                    result.coreGrammar = '过去完成时表示在过去某一时间或动作之前已经发生的动作，结构为 had + 过去分词。';
                } else if (/\bwill\s+\w+\b/.test(lower) || /\b(shall)\s+\w+\b/.test(lower) || /\b(am|is|are)\s+going\s+to\s+\w+\b/.test(lower)) {
                    result.tense = '一般将来时';
                ...
                } else if (/\b(yesterday|last\s|ago|in\s+\d{4}|when\s+i\s+was)\b/.test(lower) || /\b(went|ate|saw|did|had|took|made|bought|ran|came|gave|told|wrote|read|got|found|thought|knew|felt|became|left|spoke|began|brought|chose|drove|fell|flew|forgot|grew|lost|paid|rode|said|sat|spent|stood|stole|swam|tore|threw|woke|won)\b/.test(lower)) {
                    result.tense = '一般过去时';
                    result.coreGrammar = '一般过去时表示过去发生的动作或状态，动词用过去式。';
                } else {
                    result.tense = '一般现在时';
                    result.coreGrammar = '一般现在时表示习惯、事实或状态，注意第三人称单数动词加 -s/-es。';
                }
                ...
                if (lower.endsWith('?')) {
                    if (/^(what|where|when|why|how|who|whom|whose|which)\b/.test(lower)) {
                        ...
                        result.wordOrder = '疑问词/助动词 + 主语 + 谓语 + 其他';
                    }
                } else if (/^\w+\s+\w+\b.*\bif\b.*\b(would|will|could|can|might|may)\b/.test(lower) || /^if\b.*\b\w+\b.*,\s*\w+\b.*\b(would|will|could|can|might|may)\b/.test(lower)) {
                    result.sentenceType = '条件句';
                    result.wordOrder = 'If 从句（条件）+ 主句（结果），或主句 + if 从句';
                } else if (/^please\b|\b(don'?t|do not|never|always|try|make|let|help|remember|forget)\b/.test(lower)) {
                    result.sentenceType = '祈使句 / 建议句';
                    result.wordOrder = '（省略主语 you）+ 动词原形 + 宾语/状语';
                } else if (/\b(because|since|as)\b/.test(lower) && /,/.test(lower)) {
                    result.sentenceType = '含原因/让步状语从句的复合句';
                    result.wordOrder = '主句 + 从属连词 + 从句（顺序可调整）';
                } else if (/,/.test(lower) && /\b(and|but|or|so|yet|for|nor)\b/.test(lower)) {
                    result.sentenceType = '并列复合句';
                    result.wordOrder = '分句1 + 并列连词 + 分句2';
                } else {
                    result.sentenceType = '陈述句';
                    result.wordOrder = '主语 + 谓语 + 宾语/表语/状语（基本语序）';
                }

                // 主干成分：仅对较简单句给出大致划分
                const words = en.trim().split(/\s+/);
                if (words.length >= 2 && words.length <= 12 && !lower.endsWith('?') && (lower.match(/,/g) || []).length <= 1) {
                    result.components = [
                        { role: '主语 / 话题 (Subject/Topic)', text: words[0] },
                        { role: '谓语 / 述语 (Predicate)', text: words.slice(1).join(' ') }
                    ];
                }
            ...
            function generateExamples(en) {
                ...
                if (/\bwent\b|\bwatched\b|\bhad\b/.test(lower)) {
                    return [
                ...
                if (/\b(with|and)\b/.test(lower) && !/\b(going|eating|drinking|running)\b/.test(lower)) {
                    return [
                        { en: 'I sing and dance with my friend.', zh: '我和朋友唱歌跳舞。' },
                        { en: 'She draws and colors with her sister.', zh: '她和姐姐画画涂色。' },
                        { en: 'He jumps and laughs with his dog.', zh: '他和他的狗蹦跳大笑。' }
                    ];
                }
                if (/\b(is|am|are|was|were)\s+\w+ing\b/.test(lower)) {
                    return [
                        { en: 'She is reading a book now.', zh: '她现在正在看书。' },
                        { en: 'They are playing soccer in the park.', zh: '他们正在公园踢足球。' },
                        { en: 'He is writing a letter to his mom.', zh: '他正在给妈妈写信。' }
                    ];
                }
                if (/\b(will|shall|can|could|may|might|must|have|has|had)\b/i.test(lower)) {
                    return [
                        { en: 'I can help you with your homework.', zh: '我可以帮你做作业。' },
                        { en: 'She will visit her grandma next week.', zh: '她下周会去看奶奶。' },
                        { en: 'We must finish this project by Friday.', zh: '我们必须在周五前完成这个项目。' }
                    ];
                }
                return [
            ...
            function generateFullLesson(zh, en, grammarMatches = []) {
                ...
                const sanityWarnings = sanityCheck(zh, en);
                let html = '';

                html += `<div class="eg-block"><span class="eg-label">📌 中文原句：</span>${escapeHtml(zh)}<br><span class="eg-label">🌐 推荐译文：</span><span class="eg-en">${escapeHtml(en)}</span></div>`;

                // 语法检查
                html += `<div class="eg-block"><span class="eg-label">🔍 联网语法与用词检查：</span>`;
                if (grammarMatches.length || sanityWarnings.length) {
                    grammarMatches.forEach(m => {
                        const context = m.context ? m.context.text : '';
                        const sug = (m.replacements || []).slice(0, 3).map(r => r.value).join(' / ') || '—';
                        html += `<div class="eg-grammar-match"><span class="eg-grammar-issue">• ${escapeHtml(m.message)}</span><span class="eg-grammar-suggestion">建议：${escapeHtml(sug)}</span>${context ? `<span class="eg-grammar-context">（上下文：${escapeHtml(context)}）</span>` : ''}</div>`;
                    });
                    sanityWarnings.forEach(w => {
                        html += `<div class="eg-grammar-match eg-warning">• ${escapeHtml(w)}</div>`;
                    });
                } else {
                ...
                // 结构分析
                html += `<div class="eg-block"><span class="eg-label">📐 句型判定：</span>${escapeHtml(analysis.sentenceType)}<br>`;
                html += `<span class="eg-label">⏳ 时态：</span>${escapeHtml(analysis.tense)}<br>`;
                html += `<span class="eg-label">🌱 主干：</span><span class="eg-en">${escapeHtml(analysis.bareSentence)}</span><br>`;
                html += `<span class="eg-label">🔁 基本语序：</span>${escapeHtml(analysis.wordOrder)}<br>`;
                html += `<span class="eg-label">📖 核心语法：</span>${escapeHtml(analysis.coreGrammar)}<br>`;

                html += `<span class="eg-label">🧩 成分拆解：</span><br>`;
                if (analysis.components.length > 0) {
                    analysis.components.forEach(comp => {
                        html += `<div class="eg-component-line">• <strong>${escapeHtml(comp.role)}</strong>：${escapeHtml(comp.text)}</div>`;
                    });
                } else {
                    html += `<div class="eg-component-line">• 该句结构较复杂，暂不做机械拆分。建议结合「主干」和「语序」理解整体结构。</div>`;
                }
                html += `</div>`;

                // 例句
                const examples = generateExamples(en);
                html += `<div class="eg-block"><span class="eg-label">✍️ 同结构替换练习</span><br>`;
                examples.forEach((ex, i) => {
                    html += `<div class="eg-example"><span class="eg-en">${i+1}. ${escapeHtml(ex.en)}</span><br><span class="eg-zh">${escapeHtml(ex.zh)}</span></div>`;
                });
                html += `</div>`;

                html += `<div class="eg-block eg-text-muted">💡 译文来自 MyMemory 在线翻译，语法检查来自 LanguageTool。如果你发现译文有歧义，可把句子拆成更短的小句再试，或查阅权威词典进一步确认。</div>`;
            ...
            async function runLesson() {
                ...
                lessonOutput.innerHTML = `<div class="eg-empty">⏳ 正在联网生成逐句精讲，请稍候…</div>`;
                try {
                    ...
                    lessonOutput.innerHTML = `<div class="eg-empty">⚠️ 联网分析失败，请检查网络后重试。</div>`;
                }
            }
