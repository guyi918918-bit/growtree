...
function initEnglishPlayground() {
            ...
            // 百度翻译 API 官方不支持浏览器 CORS，必须通过公共代理转发
            // 自动尝试多个代理，任一成功即返回（个人使用，翻译内容不敏感）
            async function fetchBaiduAPI(zh, appid, key) {
                ...
                for (const wrap of proxies) {
                    try {
                        const res = await fetchWithTimeout(wrap(baiduUrl));
                        const data = await res.json();
                        let result = data;
                        if (data && data.contents) {
                            try { result = JSON.parse(data.contents); } catch (e) { result = data; }
                        }
                        if (result && result.trans_result && Array.isArray(result.trans_result) && result.trans_result.length) {
                            const trans = result.trans_result.map(t => t.dst || '').join('');
            ...
            (function setupBaiduApiSettings() {
                ...
                appidEl.value = localStorage.getItem('gt_baidu_appid') || '20260820002669847';
                keyEl.value = localStorage.getItem('gt_baidu_key') || 'MoOfiao0rIDuxeYXJwbR';
                        ...
                        if (trans) {
                            ...
                            try {
                                ...
                                if (data && data.error_code) {
                                    ...
                                    showStatus('❌ 所有 CORS 代理都不可用，且直接调用被浏览器拒绝。建议装 Chrome 扩展 "Allow CORS" 或换用其他浏览器。', 'err');
                                }
                            } catch (e) {
