...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824ad.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824ad.css'), 'utf8');

const BUILD = '20260824ad';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 96) ok('SW CACHE 已递增到 v96'); else no('SW CACHE 未递增');
...
// 健康减脂微调回归：删除 CNN / 趋势 tab / 折叠 / 体重一次性同步
try {
    // 1) 已删除 CNN 国际新闻按钮（源码层面确认无 cnn 残留）
    if (!APP_JS.includes('cnn-world') && !APP_JS.includes('CNN 国际')) ok('已删除 CNN 国际新闻按钮（无 cnn 残留）');
    else no('CNN 国际新闻按钮仍残留');

    // 2) 已删除健康减脂「趋势」tab（FATLOSS_TABS 不再含 fl-trend）
    if (!APP_JS.includes("'fl-trend'")) ok('已删除健康减脂「趋势」tab（FATLOSS_TABS 无 fl-trend）');
    else no('趋势 tab 仍残留');
    const trendFallback = run('renderFatlossPage("fl-trend")');
    if (typeof trendFallback === 'string' && trendFallback.indexOf('fl-overview') !== -1) ok('趋势 tab 访问回退到总览页');
    else no('趋势 tab 回退异常');

    // 3) 打卡记录 / 阶梯进度 折叠：默认 打卡记录展开、阶梯进度折叠
    const histCollapsed = run('flIsCollapsed("history")');
    const stepsCollapsed = run('flIsCollapsed("steps")');
    if (histCollapsed === false && stepsCollapsed === true) ok('折叠默认值正确：打卡记录展开、阶梯进度折叠');
    else no('折叠默认值异常：' + JSON.stringify({ histCollapsed, stepsCollapsed }));

    // 4) 体重打卡一次性同步：输入 118.2 应直接把当前目标台阶推到 118.0（无需逐阶点击）
    run(`
        state.data.fatloss = state.data.fatloss || {};
        state.data.fatloss.user = { name: '测试', gender: '男', startWeight: 120, targetWeight: 118, stepSize: 0.2, motto: '' };
        state.data.fatloss.stepRecords = [];
        state.data.fatloss.diet = state.data.fatloss.diet || { meals: [], checkinEmojis: {}, recipes: [] };
        state.data.fatloss.achievements = state.data.fatloss.achievements || [];
        var wInput = document.getElementById('flTodayWeight');
        if (!wInput) { wInput = document.createElement('input'); wInput.id = 'flTodayWeight'; document.body.appendChild(wInput); }
        wInput.value = '118.2';
        wInput.setAttribute('value', '118.2');
        flHandleCheckin();
        var recs = state.data.fatloss.stepRecords;
        var _steps = flGenerateSteps(120, 118, 0.2);
        var _cur = flGetCurrentStep(_steps, recs);
    `);
    const recCount = run('recs.length');
    const curTarget = run('_cur ? _cur.target : null');
    if (recCount === 10 && Math.abs(curTarget - 118.0) < 1e-9) {
        ok('体重打卡一次性同步：输入 118.2 → 一次覆盖 10 个阶梯，当前目标直接到 118.0');
    } else {
        no('体重一次性同步异常：' + JSON.stringify({ recCount, curTarget }));
    }

    // 5) 折叠点击真实生效（修复：toggle 改为调用 render() 而非只返回字符串）
    try {
        run(`
            state.data.fatloss = state.data.fatloss || {};
            state.data.fatloss.user = { name: '测试', gender: '男', startWeight: 120, targetWeight: 118, stepSize: 0.2, motto: '' };
            ...
            state.data.fatloss.achievements = state.data.fatloss.achievements || [];
            state.currentModule = 'fatloss';
            state.currentSubTab = { fatloss: 'fl-overview' };
            state._flCollapsed = { history: false, steps: true };
            render();
        `);
        run('initEvents()'); // 注册 document 点击事件（jsdom 下 DOMContentLoaded 未触发，需手动注册）
        const before = document.getElementById('content').innerHTML;
        const stepsTitle = document.querySelector('[data-action="fl-toggle-section"][data-section="steps"]');
        if (!stepsTitle) {
            no('折叠点击测试：阶梯进度标题未渲染');
        } else {
            stepsTitle.click();
            const after = document.getElementById('content').innerHTML;
            const wasCollapsed = before.indexOf('fl-section collapsed" data-section="steps"') !== -1;
            const stillThere = after.indexOf('data-section="steps"') !== -1;
            const nowExpanded = after.indexOf('fl-section collapsed" data-section="steps"') === -1;
            const stepsTitle2 = document.querySelector('[data-action="fl-toggle-section"][data-section="steps"]');
            stepsTitle2.click();
            const after2 = document.getElementById('content').innerHTML;
            const backCollapsed = after2.indexOf('fl-section collapsed" data-section="steps"') !== -1;
            if (wasCollapsed && stillThere && nowExpanded && backCollapsed) {
                ok('折叠点击真实生效：点一下展开阶梯进度、再点一下收起（render 已重绘 DOM）');
            } else {
                no('折叠点击未生效：' + JSON.stringify({ wasCollapsed, stillThere, nowExpanded, backCollapsed }));
            }
        }
    } catch (e2) {
        no('折叠点击测试异常：' + (e2 && e2.message));
    }
} catch (e) {
    no('健康减脂微调回归测试异常：' + (e && e.message));
}
