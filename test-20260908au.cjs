...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260908au.js'), 'utf8');
...
// 1. 版本号
if (VERSION.build === '20260908au') ok('version.json = au'); else no('version.json 不是 au: ' + VERSION.build);
if (APP_JS.includes("const APP_BUILD = '20260908au';")) ok('APP_BUILD = au'); else no('APP_BUILD 不是 au');
if (SW_JS.includes("CACHE = 'growtree-shell-v113'") && SW_JS.includes('app.20260908au.js') && SW_JS.includes('styles.20260908au.css')) ok('sw.js v113 引用 au'); else no('sw.js 引用异常');
...
if (YJXS_HTML.includes('time:nowHM()')) ok('showDayDetail 调用 nowHM'); else no('showDayDetail 未调用 nowHM');
...
setTimeout(() => {
  try {
    ...
    // 测试预计天数（昨日有记录会产生 cum=125，remain=6035 → ceil(6035/125)=49）
    ...
    if (grid) {
      ...
      if (recFound) ok('9/9 有记录格子渲染了 stats'); else no('9/9 有记录格子未渲染 stats');
    } else {
      no('calGrid 未生成');
    }
  } catch (e) {
