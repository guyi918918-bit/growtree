...
function ensureDefaults() {
    ...
    // 「其他打卡项」一键对齐：每台设备首次加载 v20260818i 时自动清空原有 module=='other' 项
    // （含回收站），写入 purgedCheckInIds 防止跨设备复活，然后按新版 DEFAULT_CHECKINS.other 重建。
    // 之后云端同步会把新列表推到另一台设备，两端自动对齐到完全相同的列表。
    try {
        if (!localStorage.getItem('growtree_other_v20260818i_done')) {
            const removedIds = [];
            (state.data.checkIns || []).filter(c => c.module === 'other').forEach(c => removedIds.push(c.id));
            (state.data.checkInBin || []).filter(c => c.module === 'other').forEach(c => removedIds.push(c.id));
            state.data.checkIns = (state.data.checkIns || []).filter(c => c.module !== 'other');
            state.data.checkInBin = (state.data.checkInBin || []).filter(c => c.module !== 'other');
            state.data.purgedCheckInIds = Array.from(new Set([...(state.data.purgedCheckInIds || []), ...removedIds]));
            const baseOrder = (state.data.checkIns || []).filter(c => c.module === 'other').length;
            (DEFAULT_CHECKINS.other || []).forEach((item, i) => {
                state.data.checkIns.push({
                    id: uuid(),
                    module: 'other',
                    category: cats.other || '其他打卡项',
                    name: item.name,
                    stars: item.stars,
                    points: item.points,
                    order: baseOrder + i,
            ...
            localStorage.setItem('growtree_other_v20260818i_done', '1');
        }
    } catch (e) {}
}
    ...
    return `
        <div class="card">
            ...
            <div class="manage-bar">
                ...
                <input type="number" class="input" id="managePoints" placeholder="统一设置积分" style="flex:1;min-width:90px">
            ...
            <div class="manage-bar" style="margin-top:0;margin-bottom:18px;">
                <select class="input" id="manageTargetCategory" style="flex:1;min-width:110px">
                    <option value="">移动到板块…</option>
                    ${order.map(k => `<option value="${escapeHtml(k)}">${escapeHtml(cats[k] || k)}</option>`).join('')}
                </select>
                <button class="btn btn-secondary" data-action="manage-batch-category">调整所选分组</button>
                <select class="input" id="manageTargetStars" style="flex:1;min-width:110px">
                    <option value="">统一花朵…</option>
                    ${[1,2,3,4,5].map(n => `<option value="${n}">${n} 朵 · ${flowerLevel(n).slogan} · ${flowerScore(n)} 分</option>`).join('')}
                </select>
                <button class="btn btn-secondary" data-action="manage-batch-stars">调整所选花朵</button>
            </div>
