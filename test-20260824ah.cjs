...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824ah.js'), 'utf8');
const HTML = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
const VERSION = JSON.parse(fs.readFileSync(path.join(ROOT, 'version.json'), 'utf8'));
const SW = fs.readFileSync(path.join(ROOT, 'sw.js'), 'utf8');
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824ah.css'), 'utf8');

const BUILD = '20260824ah';
let fail = 0;
...
if (swCache && parseInt(swCache[1], 10) === 100) ok('SW CACHE 已递增到 v100'); else no('SW CACHE 未递增');
...
// 8. 回归：Store.get 对 localStorage 里的 "null"/"undefined" 字符串回退到默认值
if (YJXS_HTML.includes('p==null') || YJXS_HTML.includes('p === null') || YJXS_HTML.includes('p == null')) ok('Store.get 已增强：解析为 null/undefined 时回退默认值'); else no('Store.get 未增强 null 回退');

// 9. 回归：yjxsImportAll 不会用远程 null 覆盖本地有效数据
if (YJXS_HTML.includes('remote===null')) ok('yjxsImportAll 已加入 null 覆盖防护'); else no('yjxsImportAll 缺少 null 覆盖防护');

// 10. 回归：饮食情绪选择有视觉反馈
if (YJXS_HTML.includes('state.fmFilter')) ok('renderFoodmood 使用 state.fmFilter'); else no('renderFoodmood 未使用 fmFilter');
if (YJXS_HTML.includes('selectedEm=state.fmFilter') || YJXS_HTML.includes('selectedEm===ems')) ok('饮食情绪选中状态可高亮'); else no('饮食情绪选中未高亮');
