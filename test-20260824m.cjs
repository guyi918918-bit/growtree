...
const APP_JS = fs.readFileSync(path.join(ROOT, 'app.20260824m.js'), 'utf8');
...
if (swCache && parseInt(swCache[1], 10) === 79) ok('SW CACHE 已递增到 v79'); else no('SW CACHE 未递增');
...
global.localStorage.setItem('engPlay_search', JSON.stringify([
    { query: '第一条', time: nowIso },
    { query: '第二条', time: nowIso }
]));
global.localStorage.setItem('engPlay_copyCount', '2');
global.localStorage.setItem('engPlay_copy', JSON.stringify([
    { zh: '中文A', en: 'enA', time: nowIso },
    { zh: '中文B', en: 'enB', time: nowIso }
]));
run(`ensureGameDefaults();`);
run(`engDeleteHistoryItem('search', 0);`);
const searchAfter = JSON.parse(global.localStorage.getItem('engPlay_search') || '[]');
if (searchAfter.length === 1 && searchAfter[0].query === '第二条') ok('单条删除搜索历史后保留正确项'); else no(`搜索历史单条删除异常：${JSON.stringify(searchAfter)}`);
run(`engDeleteHistoryItem('copy', 1);`);
const copyAfter = JSON.parse(global.localStorage.getItem('engPlay_copy') || '[]');
const copyCountAfter = global.localStorage.getItem('engPlay_copyCount');
if (copyAfter.length === 1 && copyAfter[0].zh === '中文A' && copyCountAfter === '1') ok('单条删除抄写历史后保留正确项且计数减一'); else no(`抄写历史单条删除异常：${JSON.stringify(copyAfter)}, count=${copyCountAfter}`);
...
// 7. 设置页 resetPointsToTarget 会同步清空已使用积分
run(`
    ...
    state.data.game.rankStars = 999;
    state.data.game.accountedEarnedStars = 999;
    state.data.game._achievementsFrozen = false;
...
// 9. 历史详情表格样式存在
const CSS = fs.readFileSync(path.join(ROOT, 'styles.20260824m.css'), 'utf8');
if (CSS.includes('.eng-history-detail .eng-manual-table') && CSS.includes('table-layout:auto') && CSS.includes('min-width:520px')) ok('历史详情表格对齐/自适应样式已添加'); else no('历史详情表格样式缺失');
