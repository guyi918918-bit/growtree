...
function engParseSentence(en) {
    ...
    const timeRegex = /^(Yesterday|Today|Tomorrow|Last\s+\w+|Next\s+\w+|\w+\s+ago|In\s+\w+|On\s+\w+)\s*[,]?\s*/i;
    let timeMatch = clean.match(timeRegex);
    let rest = clean;
    if (timeMatch) {
        result.adverbial = timeMatch[1];
        result.advType = '时间';
        rest = clean.slice(timeMatch[0].length);
    }
    let subjectMatch = rest.match(/^(my\s+husband\s+and\s+i|my\s+wife\s+and\s+i|my\s+friend\s+and\s+i|my\s+friends\s+and\s+i|i|you|he|she|we|they|my\s+\w+)\s*/i);
    let subject = '';
    if (subjectMatch) {
        subject = subjectMatch[1];
        rest = rest.slice(subjectMatch[0].length);
    } else {
        let words = rest.split(' ');
        if (words.length > 0) { subject = words[0]; rest = words.slice(1).join(' '); }
    }
    if (subject) result.subject = subject;

    let verbMatch = rest.match(/^(went\s+to\s+eat|went\s+to\s+have|want\s+to\s+eat|want\s+to\s+have|want\s+to|went\s+mountain\s+climbing|went|had|ate|have|eat|like|love|enjoy|is\s+reading|drinks|runs)\s*/i);
    let verb = '';
    let remainingObj = rest;
    if (verbMatch) {
        verb = verbMatch[1];
        remainingObj = rest.slice(verbMatch[0].length);
    } else {
        let words = rest.split(' ');
        if (words.length > 0) { verb = words[0]; remainingObj = words.slice(1).join(' '); }
    }
    if (verb) result.verb = verb;

    let obj = remainingObj.trim();
    let prepMatch = obj.match(/\s+(with|at|in|on|for|to)\s+.*$/i);
    if (prepMatch) {
        const advPhrase = prepMatch[0].trim();
        obj = obj.replace(prepMatch[0], '').trim();
        if (result.adverbial === '—') {
            result.adverbial = advPhrase;
            result.advType = '方式';
        } else {
            result.modifier = advPhrase;
        }
    }
    if (obj) result.object = obj;
    if (result.adverbial === '' || result.adverbial === null) result.adverbial = '—';
    return result;
}

const ENG_ZH_MAP = {
    'yesterday': '昨天', 'today': '今天', 'tomorrow': '明天', 'last month': '上个月',
    'next month': '下个月', 'last week': '上周', 'next week': '下周', 'this week': '本周',
    'last year': '去年', 'next year': '明年', 'i': '我', 'you': '你', 'he': '他',
    'she': '她', 'we': '我们', 'they': '他们', 'my husband and i': '我和我老公',
    'my wife and i': '我和我老婆', 'my friend and i': '我和我的朋友',
    'my friends and i': '我和我的朋友们', 'my family and i': '我和我的家人',
    'my husband': '我老公', 'my wife': '我老婆', 'my friend': '我的朋友',
    'my friends': '我的朋友们', 'my family': '我的家人', 'my sister': '我姐姐',
    'my brother': '我哥哥', 'my mom': '我妈妈', 'my dad': '我爸爸',
    'went to eat': '去吃', 'went to have': '去吃', 'went mountain climbing': '去爬山',
    'went': '去', 'want to have': '想请/想吃', 'want to': '想', 'had': '吃了',
    'ate': '吃了', 'have': '有/吃', 'eat': '吃', 'like': '喜欢', 'love': '爱',
    'enjoy': '享受', 'is reading': '正在读', 'drinks': '喝', 'runs': '跑',
    'with a friend': '和朋友一起', 'at my place': '在我家', 'at home': '在家',
    'a delicious snack': '一个很好吃的零食', 'a delicious hot pot': '好吃的火锅',
    'delicious': '好吃的', 'mountain climbing': '爬山'
};
...
function engGenerateOutput(zh, en) {
    ...
    const parsed = engParseSentence(cleanEn);
    const zhSubject = engTranslateToChinese(parsed.subject);
    const zhVerb = engTranslateToChinese(parsed.verb);
    const zhObject = engTranslateToChinese(parsed.object);
    const zhModifier = parsed.modifier !== '—' ? engTranslateToChinese(parsed.modifier) : '—';
    const zhAdverbial = parsed.adverbial !== '—' ? engTranslateToChinese(parsed.adverbial) : '—';
    const zhComplement = '—';
    let advHeader = '状语';
    if (parsed.advType) advHeader += `（${parsed.advType}）`;
    let html = '';
    html += `<div class="eng-lesson-line"><span class="eng-label">第一句（中文原句）</span><span class="eng-content">${zh}</span></div>`;
    html += `<div class="eng-lesson-line"><span class="eng-label">第二句（英文翻译）</span><span class="eng-en">${cleanEn}</span></div>`;
    html += `<div class="eng-table-wrap"><table><thead><tr><th>主语</th><th>谓语动词</th><th>宾语/表语</th><th>定语/修饰语</th><th>${advHeader}</th><th>补足语</th></tr></thead><tbody>`;
    html += `<tr><td>${parsed.subject}</td><td>${parsed.verb}</td><td>${parsed.object}</td><td>${parsed.modifier}</td><td>${parsed.adverbial}</td><td>${parsed.complement}</td></tr>`;
    html += `<tr><td>${zhSubject}</td><td>${zhVerb}</td><td>${zhObject}</td><td>${zhModifier}</td><td>${zhAdverbial}</td><td>${zhComplement}</td></tr>`;
