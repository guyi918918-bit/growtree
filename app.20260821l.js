...
async function engTranslateAPI(zh) {
    // 第1层：百度翻译 API（用户已配置 key 时最准确）
    const { appid, secret, apiKeys } = engGetBaiduCreds();
    if (appid && secret) {
        const r = await engFetchBaidu(appid, secret, zh);
        if (r && !/^\[Translation\]/.test(r)) return r;
    }
    // 第2层：API Keys（可选）——支持 OpenAI 兼容接口，格式：密钥 或 openai|密钥|https://api地址（国内镜像如 https://api.deepseek.com 也可）
    if (apiKeys) {
    ...
    // 第3层：MyMemory（免费、无需 key）
    const myMemoryUrl = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(zh)}&langpair=zh|en&de=demo@example.com`;
    const r2 = await engFetchWithProxies(myMemoryUrl, d => d && d.responseData && d.responseData.translatedText, 7000);
    if (r2) return r2;
    // 第4层：LibreTranslate 公共实例（免费、无需 key）
    const ltUrl = `https://libretranslate.com/translate?q=${encodeURIComponent(zh)}&source=zh&target=en`;
    const r3 = await engFetchWithProxies(ltUrl, d => d && d.translatedText, 7000);
    if (r3) return r3;
    // 第5层：内置词典 / 模板
...
function engRenderHome() {
    ...
    <div class="eng-card">
        ...
        <div class="eng-text-muted" style="margin-bottom:8px;">🌐 输入中文后自动联网翻译（百度优先，免费接口兜底）；想更准可到「更多→翻译 API 设置」填百度密钥</div>
        <div class="eng-row">
