...
async function engTranslateAPI(zh) {
    ...
    const work = (async () => {
        // 第1层：Google Translate（走 CORS 代理，免 key，国内质量最稳）
        const g = await engFetchGoogle(zh);
        if (g) return g;
        // 第2层：百度翻译 API（用户配了 key 时最准）
        ...
        // 第3层：API Keys（OpenAI 兼容接口，可填国内镜像如 deepseek）
        if (apiKeys) {
        ...
        // 第4层：MyMemory（免费、无需 key，但质量一般）
        const myMemoryUrl = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(zh)}&langpair=zh|en&de=demo@example.com`;
        const r2 = await engFetchWithProxies(myMemoryUrl, d => d && d.responseData && d.responseData.translatedText, 5000);
        if (r2) return r2;
        // 第5层：LibreTranslate 公共实例（免费、无需 key）
        const ltUrl = `https://libretranslate.com/translate?q=${encodeURIComponent(zh)}&source=zh&target=en`;
        const r3 = await engFetchWithProxies(ltUrl, d => d && d.translatedText, 5000);
        if (r3) return r3;
        // 第6层：内置词典 / 模板
...
function engGenerateOutput(zh, colCount) {
    // 默认 8 列，3 行：英文 / 中文 / 备注
    colCount = Math.max(3, Math.min(20, parseInt(colCount, 10) || 8));
    const rowLabels = ['英文（用户填）', '中文（用户填）', '备注（用户填）'];
    const defaultNotes = ['主语', '谓语', '宾语', '定语', '状语（时间）', '状语（地点）', '状语（方式）', '补足语'];
    ...
    html += `<th class="eng-corner-cell">操作栏</th>`;
    for (let i = 0; i < colCount; i++) {
        html += `<th class="eng-col-head">
            <span>列 ${i + 1}</span>
            <button class="eng-del-col" data-action="eng-remove-column" data-col="${i}" title="删除此列">✖</button>
        </th>`;
    }
    html += `</tr></thead><tbody>`;
    ...
    for (let r = 0; r < 3; r++) {
        html += `<tr>`;
        html += `<td class="eng-row-label">${rowLabels[r]}</td>`;
        for (let c = 0; c < colCount; c++) {
            const placeholder = r === 0 ? 'English' : (r === 1 ? '中文' : defaultNotes[c] || '备注');
            const val = r === 2 ? (defaultNotes[c] || '') : '';
            html += `<td class="eng-edit-cell">
                <input type="text" class="eng-cell-input" data-row="${r}" data-col="${c}" placeholder="${placeholder}" value="${escapeHtml(val)}">
            </td>`;
        }
        html += `</tr>`;
    }
    ...
    html += `<div class="eng-table-actions">
        <button class="eng-btn eng-btn-outline eng-btn-sm" data-action="eng-add-column">➕ 增加列</button>
        <span class="eng-text-muted">列数可自由增减，长按/滑动表格可横向滚动</span>
    </div>`;
...
async function engRunLesson() {
    ...
    output.innerHTML = `<div class="eng-empty">⏳ 正在生成表格...</div>`;
    const html = engGenerateOutput(zh);
    engCurrentLesson = { zh, en: '', fullHtml: html };
    output.innerHTML = html;
    engAddHistory('copy', { time: new Date().toISOString(), zh: zh, en: '', detail: html });
    const todayStats = engGetTodayStats();
...
function engSerializeTable() {
    ...
    const colCount = table.querySelectorAll('thead th').length - 1; // 去掉操作栏
    const rows = [[], [], []];
    ...
    return { colCount, rows, en: enSentence ? enSentence.textContent.trim() : '' };
}
...
function engRebuildTableWithCols(colCount) {
    ...
    // 备注行默认值：如果为空则填入默认语法角色
    const defaultNotes = ['主语', '谓语', '宾语', '定语', '状语（时间）', '状语（地点）', '状语（方式）', '补足语'];
    for (let c = 0; c < colCount; c++) {
        if (!newRows[2][c]) newRows[2][c] = defaultNotes[c] || '备注';
    }
    // 重新生成表格
    const output = document.getElementById('engLessonOutput');
    if (!output) return;
    output.innerHTML = engGenerateOutput(zh, colCount);
    // 回填
    const table = document.getElementById('engManualTable');
    const trs = table.querySelectorAll('tbody tr');
    trs.forEach((tr, r) => {
...
function engAddColumn() {
    const table = document.getElementById('engManualTable');
    if (!table) return;
    const currentCols = table.querySelectorAll('thead th').length - 1;
    if (currentCols >= 20) {
        engToast('最多 20 列，已经足够长啦');
        return;
    }
    engRebuildTableWithCols(currentCols + 1);
}

function engRemoveColumn(colIdx) {
    const table = document.getElementById('engManualTable');
    if (!table) return;
    const currentCols = table.querySelectorAll('thead th').length - 1;
    if (currentCols <= 3) {
    ...
    for (let r = 0; r < 3; r++) {
        for (let c = 0; c < currentCols; c++) {
            ...
            newRows[r].push(old.rows[r] && old.rows[r][c] !== undefined ? old.rows[r][c] : '');
        }
    }
    engRebuildTableWithCols(currentCols - 1);
    // 回填
...
function engRenderHome() {
    ...
    <div class="eng-card">
        ...
        <div class="eng-text-muted" style="margin-bottom:8px;">🧪 用法：输入一句中文 → 点「生成填表」→ 在 8 列表格里按英文语序自行对应填空</div>
        <div class="eng-row">
...
function initEvents() {
    ...
    document.addEventListener('click', async e => {
        ...
        if (action === 'eng-add-column') {
            engAddColumn();
            return;
        }
        if (action === 'eng-remove-column') {
            const idx = parseInt(el.dataset.col, 10);
            if (!isNaN(idx)) engRemoveColumn(idx);
            return;
        }
        if (action === 'eng-copy') {
            engCopyCurrent();
