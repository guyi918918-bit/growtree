...
function engGenerateOutput(zh, en) {
    let cleanEn = en.trim();
    if (!/[.?!]$/.test(cleanEn)) cleanEn += '.';
    // 铁律：先分句——一句中文对应一句英文，分句之间严格分开
    const zhSentences = engSplitSentences(zh, 'zh');
    const enSentences = engSplitSentences(cleanEn, 'en');
    const n = Math.max(zhSentences.length, enSentences.length);

    const cell = v => v === '—' || !v ? '—' : escapeHtml(v);
    const cols = ['subject','verb','object','modifier','advTime','advPlace','advManner','complement'];
    const heads = ['主语','谓语动词','宾语/表语','定语','状语（时间）','状语（地点）','状语（方式）','补足语'];

    let html = '';
    html += `<div class="eng-lesson-line"><span class="eng-label">第一句（中文原句）</span><span class="eng-content">${escapeHtml(zh)}</span></div>`;
    html += `<div class="eng-lesson-line"><span class="eng-label">第二句（英文翻译）</span><span class="eng-en">${escapeHtml(cleanEn)}</span></div>`;
    ...
    for (let i = 0; i < n; i++) {
        ...
        html += `</tr>`;
        // 中文行（与英文行同列对齐）：先查表，未翻出的标记 data-phrase 由 engTranslatePendingCells 异步补齐
        html += `<tr>`;
        for (const k of cols) {
            const enVal = pEn[k];
            if (!enVal || enVal === '—') {
            ...
            if (t && t !== enVal && !/[a-zA-Z]{2,}/.test(t)) {
                html += `<td>${escapeHtml(t)}</td>`;
            } else {
                html += `<td class="eng-zh-pending" data-phrase="${escapeHtml(enVal)}">${escapeHtml(enVal)}</td>`;
            }
        }
        html += `</tr>`;
...
async function engRunLesson() {
    ...
    if (!zh) {
        output.innerHTML = `<div class="eng-empty">💡 输入中文句子，点击「逐句精讲」获取语法对比表</div>`;
        engCurrentLesson = { zh: '', en: '', fullHtml: '' };
        return;
    }
    output.innerHTML = `<div class="eng-empty">⏳ 正在翻译...</div>`;
    const en = await engTranslateAPI(zh);
    const html = engGenerateOutput(zh, en);
    engCurrentLesson = { zh, en, fullHtml: html };
    output.innerHTML = html;
    // 中文行未翻出的成分：异步用 Google 英→中 补齐（不阻塞显示）
    engTranslatePendingCells(output);
...
function engRenderHome() {
    ...
    <div class="eng-card">
        ...
        <div class="eng-text-muted" style="margin-bottom:8px;">🌐 输入中文后自动联网翻译（Google 翻译优先，免 key；百度/API Keys 备用）；想更准可到「更多→翻译 API 设置」填百度密钥</div>
        <div class="eng-row">
            <input type="text" id="engTranslateInput" placeholder="输入中文句子，如：上个月我和朋友去爬山了" data-action="eng-translate-input">
            <button class="eng-btn eng-btn-primary" data-action="eng-translate">📖 逐句精讲</button>
        </div>
        <div class="eng-lesson-output" id="engLessonOutput"><div class="eng-empty">💡 输入中文句子，点击「逐句精讲」获取语法对比表</div></div>
